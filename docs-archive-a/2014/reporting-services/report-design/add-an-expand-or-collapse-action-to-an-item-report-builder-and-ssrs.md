...
4.  在中 `Hidden` ，选择以下选项之一来设置首次运行报表时该报表项的可见性：  
  
    -   选择 `False` 此选项可以显示报表项。  
  
    -   选择 `True` 此选项可以隐藏报表项。  
  
    -   选择 **\<Expression>** 此复选框可以打开 "**表达式**" 对话框，以便创建在运行时计算的表达式来确定可见性。  
...
1.  在报表设计视图中，右键单击要显示或隐藏的报表项，然后单击 " *\<report item>* **属性**"。 该 *\<report item>* 报表项的 "**属性**" 对话框将打开。  
