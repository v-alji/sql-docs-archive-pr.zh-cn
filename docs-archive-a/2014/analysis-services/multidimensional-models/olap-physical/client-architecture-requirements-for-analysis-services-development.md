...
### <a name="interfaces-for-powerpivot-client"></a>PowerPivot 客户端的接口
 [!INCLUDE[ssGemini](../../../includes/ssgemini-md.md)]使用用于 Analysis Services 的已建立接口和语言，与 xVelocity 内存中分析引擎进行交互， (VertiPaq) 存储引擎。 在该外接程序内，通过使用与 Excel、数据分析表达式 (DAX) 类似的公式语言定义度量值。 DAX 表达式嵌入在发送到进程内服务器的 XMLA 消息内。

### <a name="providers"></a>提供程序
 [!INCLUDE[ssGemini](../../../includes/ssgemini-md.md)]和 Excel 之间的通信使用 MSOLAP OLEDB 访问接口 (版本 11.0) 。 在 MSOLAP 访问接口内，有四个可用于在客户端和服务器之间发送消息的不同的模块或传输。

 **TCP/IP**用于正常的客户端-服务器连接。

 **HTTP**用于通过 SSAS 数据抽取服务的 HTTP 连接，或通过调用 SharePoint PowerPivot Web Service (WS) 组件。
