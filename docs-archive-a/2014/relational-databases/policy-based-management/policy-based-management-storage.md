...
## <a name="storing-policies"></a>存储策略  
 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 包括可用于监视 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]实例的策略。 默认情况下，中未安装这些策略 [!INCLUDE[ssDE](../../includes/ssde-md.md)] ; 不过，可以从 C:\Program 文件的默认安装位置 (x86) \MICROSOFT SQL server\120\tools\policies\databaseengine\1033 导入这些策略。  
