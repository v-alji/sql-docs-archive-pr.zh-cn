...
2.  在要搜索的每个表中，按以下方法创建全文索引：  
    ...
    2.  如果给定列包含存储为二进制数据 (`varbinary(max)` 或 `image` 数据) 的文档，则必须 (*类型列*) 指定一个表列，该列标识要编制索引的列中的每个文档的类型。  
...
-   始终选择可用于全文唯一键的最小唯一索引。  (4 字节的、基于整数的索引是最佳的。 ) 这样可以 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 显著减少文件系统中 Search 服务所需的资源。 如果主键较大（超过 100 个字节），可以考虑选择表中的另一个唯一索引（或创建另一个唯一索引）来作为全文唯一键。 否则，如果全文唯一键的大小超过所允许的最大值（900 个字节），全文填充将无法继续进行。  
