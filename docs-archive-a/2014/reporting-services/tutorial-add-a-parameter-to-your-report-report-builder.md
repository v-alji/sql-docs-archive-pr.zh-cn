...
3.  将以下子句添加到 [!INCLUDE[tsql](../includes/tsql-md.md)] `WHERE` 查询中的最后一行：  
  
    ```  
    WHERE StoreID = (@StoreID)  
    ```  
  
     子句将检索到的 `WHERE` 数据限制为查询参数指定的存储区标识符 *@StoreID* 。  
...
8.  在“报表数据”窗格中，展开“参数”**** 文件夹。  
 ...
 请注意，现在有一个名为的报表参数 *@StoreID* 。 默认情况下，参数的数据类型为**Text**。 由于商店标识符是一个整数，因此在下一个过程中将数据类型更改为 Integer。  
...
1.  在 "报表数据" 窗格的 "**参数**" 节点下，右键单击 *@StoreID* ，然后单击 "**参数属性**"。  
...
9. 预览报表。  
     ...
     在报表查看器工具栏中，参数文本框现在为显示的下拉列表 **\<Select a Value>** 。  
...
3.  从 "报表数据" 窗格中，将参数拖 *@StoreID* 到文本框中。 文本框显示 `[@StoreID]`。  
...
6.  在 `equals` `IN` 查询的最后一行中的 [!INCLUDE[tsql](../includes/tsql-md.md)] 子句中，更改 (=) `WHERE` ：  
...
8.  在报表查看器工具栏中的 "**显示选定**项" 旁边，单击 `True` 。  
