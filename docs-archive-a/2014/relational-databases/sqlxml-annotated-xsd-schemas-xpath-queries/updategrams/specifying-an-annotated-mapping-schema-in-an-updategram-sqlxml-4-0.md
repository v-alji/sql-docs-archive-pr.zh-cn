...
## <a name="dealing-with-data-types"></a>处理数据类型  
 如果架构 `image` 使用) 指定、 `binary` 或 `varbinary` [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 数据类型 (`sql:datatype` 并且未指定 xml 数据类型，则 updategram 将假定 xml 数据类型为 `binary base 64` 。 如果数据类型为 `bin.base`，则必须显式指定类型（`dt:type=bin.base` 或 `type="xsd:hexBinary"`）。  
...
```  
 ...
 以下 updategram 使用此 XSD 架构为订单43860添加新的订单详细记录 (**\<OD>** 块) 中的元素 **\<after>** 。 `mapping-schema` 属性用于指定 updategram 中的映射架构。  
