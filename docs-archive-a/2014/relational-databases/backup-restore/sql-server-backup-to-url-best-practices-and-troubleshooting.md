...
-   [使用 Azure Blob 存储服务执行 SQL Server 备份和还原](sql-server-backup-and-restore-with-microsoft-azure-blob-storage-service.md)  
 ...
 **未选择默认代理设置：**  
 ...
 有时不会选取默认设置，导致出现如下代理身份验证错误：在*文件 "" 上发生不可恢复的 i/o 错误： " http://storageaccount.blob.core.windows.net/container/BackupAzurefile.bak: 备份到 URL" 收到来自远程终结点的异常。异常消息：远程服务器返回错误： (407) * **需要代理身份验证**。  
