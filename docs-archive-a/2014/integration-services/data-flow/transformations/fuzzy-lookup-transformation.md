...
## <a name="row-comparison"></a>行比较  
 ...
 如果将详尽的属性设置为 `False` ，则模糊查找转换只返回至少具有一个索引标记或子字符串的匹配项， (子字符串称为*q 语法*) 与输入记录共有。 若要最大程度提高查找效率，请以模糊查找转换查找匹配项时所用的倒排索引结构仅对表内每行中的一个令牌子集建立索引。 当输入数据集很小时，可以将 "穷举" 设置为， `True` 以避免在索引表中不存在任何公共标记的缺失匹配项。  
