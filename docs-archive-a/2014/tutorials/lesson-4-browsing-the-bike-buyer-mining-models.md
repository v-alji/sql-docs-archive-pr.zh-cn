...
## <a name="returning-the-content-of-a-mining-model"></a>返回挖掘模型的内容  
 ...
 下面是 SELECT FROM 的一般示例 \<model> 。内容语句：  
...
2.  从中复制 SELECT 的一般示例 \<model> 。内容语句插入到空白查询中。  
...
6.  在 "**另存为**" 对话框中，浏览到相应的文件夹，并将该文件命名为 `SELECT_CONTENT.dmx` 。  
...
## <a name="use-drillthrough"></a>使用钻取功能  
 ...
 下面是 SELECT FROM 的一般示例 \<model> 。Case 语句：  
...
2.  从中复制 SELECT 的一般示例 \<model> 。Case 语句插入到空白查询中。  
...
6.  在 "**另存为**" 对话框中，浏览到相应的文件夹，并将该文件命名为 `SELECT_DRILLTHROUGH.dmx` 。  
...
6.  在 "**另存为**" 对话框中，浏览到相应的文件夹，并将该文件命名为 `SELECT_DISCRETE.dmx` 。  
