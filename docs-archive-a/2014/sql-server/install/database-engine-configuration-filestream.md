...
# <a name="database-engine-configuration---filestream"></a>数据库引擎配置 - 文件流
  使用此页可针对此 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]安装启用 FILESTREAM。 FILESTREAM [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)] 通过将 `varbinary(max)` 二进制大型对象 (BLOB) 数据作为文件存储在文件系统上，将与 NTFS 文件系统集成。 [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句可插入、更新、查询、搜索和备份 FILESTREAM 数据。 Win32 文件系统接口提供对数据的流访问权限。  
