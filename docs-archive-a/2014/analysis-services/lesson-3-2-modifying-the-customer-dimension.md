...
5.  单击底部的**NameColumn**属性字段，然后单击 "浏览 () **...** " 按钮以打开 "**名称列**" 对话框。  
...
3.  在属性窗口中，单击顶部的**AttributeHierarchyDisplayFolder**属性字段 (可能需要指向它才能看到全名) ，然后键入 `Location` 。  
...
6.  若要设置属性的**namecolumn**属性 `State-Province` ，请单击 "属性窗口中的**namecolumn**字段，然后单击" 浏览 (**...**) "按钮。  
