...
-   存储过程 **sp_addpublication** 在执行 [sp_addqreader_agent (Transact-SQL)](/sql/relational-databases/system-stored-procedures/sp-addqreader-agent-transact-sql) 之前执行。 这适用于为排队更新订阅启用的事务发布， (sp_addpublication) 的参数的值为 TRUE **@allow_queued_tran** 。 **sp_addpublication**  
