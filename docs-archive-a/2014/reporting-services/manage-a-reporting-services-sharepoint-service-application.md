...
|报表生成器启动 URL|当报表服务器不使用默认报表生成器 URL 时，可指定自定义 URL。 此设置是可选的。 如果不指定值，将使用默认 URL，它将启动报表生成器。 若要启动报表生成器3.0 作为单击一次的应用程序，请输入以下值： "http:// \<computername> ReportBuilder_3_0_0_0/reportserver/reportbuilder/"。|
 ...
 **帐户** 输入一个 Windows 域用户帐户。 使用以下格式： * \<domain> \\<用户帐户 \> *。
