...
|呈现和分页方式已更改。| (ROM 中的呈现对象模型) 更改 [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 。 不再支持早期版本的呈现对象模型。 不支持从多线程呈现扩展插件中访问呈现对象模型（以及从多个线程切换上下文）。|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 2005 SP2|  
