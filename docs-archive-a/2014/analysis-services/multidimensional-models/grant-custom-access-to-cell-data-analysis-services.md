...
## <a name="set-read-contingent-permissions-on-calculated-measures"></a>设置对计算度量值的“有条件读取”权限  
 单元安全性为设置对参与计算的相关单元的权限提供了一个替代，即“有条件读取”。 再次考虑 **分销商毛利润** 示例。 输入上一节中提供的同一 MDX 表达式时，将此时间置于 "**创建角色**单元数据" 对话框的第二个文本区域  |  **Cell data**中 (在下面的文本区域中，**允许读取单元安全性) 上的单元格内容**的显示。 由于 **分销商毛利润** 视 **分销商销售额** 和 **分销商总产品成本**而定，现无法访问毛利润，因为无法访问其组成部分。  
