...
-   [常规配置设置 ( # A0) ](#bkmk_generalconfiguration)  
  
-   [URLReservations ( # A0 file) ](#bkmk_URLReservations)  
  
-   [身份验证 ( # A0 文件) ](#bkmk_Authentication)  
  
-   [Service ( # A0 file) ](#bkmk_service)  
  
-   [) 常规配置 (传递 UI 扩展](#bkmk_ui)  
  
-   [扩展 ( # A0 文件) 本机模式](#bkmk_extensions)  
  
    -   [) 常规配置的传递扩展插件 (](#bkmk_extensionsgeneral)  
...
-   [MapTileServerConfiguration ( # A0 file) ](#bkmk_MapTileServer)  
...
|**ReportBuilderTrustLevel**|请不要修改此值，它是不可配置的。 在 [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 和更高版本中，报表生成器仅在中运行 `FullTrust` 。 有关详细信息，请参阅 [配置报表生成器访问权限](configure-report-builder-access.md) 。 有关停止部分信任模式的详细信息，请参阅[SQL Server 2014 中 SQL Server Reporting Services 的功能](../discontinued-functionality-to-sql-server-reporting-services-in-sql-server.md)。|N,S|  
...
|**SendEmailToUserAlias**|此值与 **DefaultHostName**一起使用。<br /><br /> 当**SendEmailToUserAlias**设置为时 `True` ，将自动将定义单独订阅的用户指定为报表的收件人。 并隐藏 **“收件人”** 字段。 如果此值为 `False` ，则 "**至**" 字段可见。 若要最大限度地控制报表分发，请将此值设置为 `True`。 有效值包括以下值：<br /><br /> `True`=使用创建订阅的用户的电子邮件地址。 这是默认值。<br /><br /> `False`=可以指定任何电子邮件地址。|  
...
-   [呈现扩展插件的设备信息设置 (Reporting Services)](../device-information-settings-for-rendering-extensions-reporting-services.md)  
 ...
 有关下子元素的属性的信息 **\<Extension>** **\<Render>** ，请参阅以下内容：  
...
##  <a name="maptileserverconfiguration-rsreportserverconfig-file"></a><a name="bkmk_MapTileServer"></a> MapTileServerConfiguration（RSReportServer.config 文件）  
 `MapTileServerConfiguration`定义 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 必应地图 Web 服务的配置设置，这些设置为在 Report Server 上发布的报表中的地图报表项提供图块背景。 所有子元素都是必需的。  
