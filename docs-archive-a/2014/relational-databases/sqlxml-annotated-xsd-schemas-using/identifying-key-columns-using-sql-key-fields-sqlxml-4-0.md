 ...
 **\<sql:relationship>** 标记用于指定父子关系。 它将 Sales.SalesOrderHeader 表中的 CustomerID 标识为父键，该父键引用 Sales.Customer 表中的 CustomerID 子键。 中提供的信息不足 **\<sql:relationship>** 以唯一标识父表中的行， (SalesOrderHeader) 。 因此，如果没有 `sql:key-fields` 批注，则生成的层次结构不准确。  
  
 对于 `sql:key-fields` 在上指定的 **\<Order>** ，批注唯一标识父 (SalesOrderHeader 表) 中的行，其子元素显示在其父元素之下。  
