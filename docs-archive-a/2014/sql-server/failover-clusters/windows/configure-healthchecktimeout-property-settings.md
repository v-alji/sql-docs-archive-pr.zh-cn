...
# <a name="configure-healthchecktimeout-property-settings"></a>配置 HealthCheckTimeout 属性设置
  HealthCheckTimeout 设置用于指定在报告 AlwaysOn 故障转移群集实例 (FCI) 为无响应之前，SQL Server 资源 DLL 应等待[sp_server_diagnostics](/sql/relational-databases/system-stored-procedures/sp-server-diagnostics-transact-sql)存储过程返回的信息的时间长度（以毫秒为单位）。 对超时设置所做的更改会立即生效，不需要重新启动 SQL Server 资源。  
