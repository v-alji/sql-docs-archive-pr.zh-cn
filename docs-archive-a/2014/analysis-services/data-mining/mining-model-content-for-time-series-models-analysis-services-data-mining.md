...
### <a name="structure-of-an-arima-model"></a>ARIMA 模型的结构  
 ARIMA 算法为数据 (序列的每个组合（如 **[Region]**) ）和可预测 (属性（如 **[Sales]**) ）创建了一条信息，这些信息是描述可预测属性随时间变化的公式。  
...
#### <a name="mining-legend-for-an-artxp-formula"></a>ARTXP 公式的挖掘图例  
 ...
 例如，一个元素的数量为 (R250 北美，-7) 。 该表示法表示北美地区在 t-7 或当前时间段之前七个时间段的数量。 此数据序列的值乘以系数 -0.293。 各元素的系数在定型过程中导出，其基于数据中的趋势。  
