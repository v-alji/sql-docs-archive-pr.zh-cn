...
## <a name="command-options-for-unicode-native-format"></a>Unicode 本机格式的命令选项  
 可以使用**bcp**、BULK INSERT 或 INSERT ... 将 Unicode 本机格式数据导入表中\*从 OPENROWSET (BULK ... ) 中进行选择。对于**bcp**命令或 BULK INSERT 语句，可以在命令行中指定数据格式。 对于 INSERT ...SELECT * FROM OPENROWSET(BULK...) 语句，必须在格式化文件中指定数据格式。  
  
 下列选项支持 Unicode 本机格式：  
  
|Command|选项|说明|  
|-------------|------------|-----------------|  
|**bcp**|**-N**|使**bcp**实用工具使用 Unicode 本机格式，这种格式将对所有非字符数据使用本机 (数据库) 数据类型，为所有字符 (、、、、 `char` `nchar` `varchar` `nvarchar` `text` 和 `ntext`) 数据使用 unicode 字符数据格式。|  
