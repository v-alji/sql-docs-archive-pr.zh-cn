...
ms.locfileid: "87578191"
---
# <a name="persisted-data-source-objects"></a><span data-ttu-id="d6a42-102">持久化数据源对象</span><span class="sxs-lookup"><span data-stu-id="d6a42-102">Persisted Data Source Objects</span></span>
  <span data-ttu-id="d6a42-103">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Native Client OLE DB 提供程序支持具有**IPersistFile**接口的持久化数据源对象。</span><span class="sxs-lookup"><span data-stu-id="d6a42-103">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB provider supports persisted data source objects with the **IPersistFile** interface.</span></span>  
  
## <a name="examples"></a><span data-ttu-id="d6a42-104">示例</span><span class="sxs-lookup"><span data-stu-id="d6a42-104">Examples</span></span>  
 <span data-ttu-id="d6a42-105">**A. 使数据源初始化持久化：**</span><span class="sxs-lookup"><span data-stu-id="d6a42-105">**A. Persist data source initialization:**</span></span>  
  
 <span data-ttu-id="d6a42-106">此示例显示使数据源初始化属性持久化的一个函数，它为连接定义服务器、数据库并使用 Windows 身份验证模式。</span><span class="sxs-lookup"><span data-stu-id="d6a42-106">This example shows a function that persists data source initialization properties defining a server, database, and the use of the Windows Authentication Mode for connection.</span></span> <span data-ttu-id="d6a42-107">在函数的 pLocation 和 pDatasource 参数中接收服务器名称和数据库名称\*\*\*\*。</span><span class="sxs-lookup"><span data-stu-id="d6a42-107">The server name and database name are received in the *pLocation* and *pDatasource* parameters of the function.</span></span>  
...
```  
 ...
 <span data-ttu-id="d6a42-108">**B. 使用持久化的数据源初始化：**</span><span class="sxs-lookup"><span data-stu-id="d6a42-108">**B. Use persisted data source initialization:**</span></span>  
  
 <span data-ttu-id="d6a42-109">此示例使用具有其他初始化属性的一个持久化数据源对象，这些属性提供 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 登录名和密码。</span><span class="sxs-lookup"><span data-stu-id="d6a42-109">This example uses a persisted data source object with additional initialization properties that provide a [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] login and password.</span></span>  
...
```  
 ...
 <span data-ttu-id="d6a42-110">可在调用 IDBInitialize::Initialize 前后调用 IPersistFile::Save 方法\*\*\*\*\*\*\*\*。</span><span class="sxs-lookup"><span data-stu-id="d6a42-110">The **IPersistFile::Save** method can be called before or after calling **IDBInitialize::Initialize**.</span></span> <span data-ttu-id="d6a42-111">如果在从 IDBInitialize::Initialize 成功返回后调用该方法，可确保使有效的数据源指定持久化\*\*\*\*。</span><span class="sxs-lookup"><span data-stu-id="d6a42-111">Calling the method after a successful return from **IDBInitialize::Initialize** ensures a valid data source specification is persisted.</span></span>  
  
## <a name="see-also"></a><span data-ttu-id="d6a42-112">另请参阅</span><span class="sxs-lookup"><span data-stu-id="d6a42-112">See Also</span></span>  
 [<span data-ttu-id="d6a42-113">数据源对象 (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="d6a42-113">Data Source Objects &#40;OLE DB&#41;</span></span>](data-source-objects-ole-db.md)  
