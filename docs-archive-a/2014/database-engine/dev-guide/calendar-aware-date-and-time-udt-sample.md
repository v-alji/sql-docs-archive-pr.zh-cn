...
ms.locfileid: "87579738"
---
# <a name="calendar-aware-date-and-time-udt-sample"></a><span data-ttu-id="17c9e-102">日历感知日期和时间 UDT 示例</span><span class="sxs-lookup"><span data-stu-id="17c9e-102">Calendar-Aware Date and Time UDT Sample</span></span>
  <span data-ttu-id="17c9e-103">将日期作为字符串存储会导致混乱，因为在不识别所使用日历系统的情况下日期毫无意义。`CADatetime` 示例定义两个用户定义数据类型（`CADatetime` 和 `CADate`），通过它们可以按日历方式对日期和时间进行处理。</span><span class="sxs-lookup"><span data-stu-id="17c9e-103">Storing dates as strings can be confusing because dates are meaningless without understanding what calendar system is being used.The `CADatetime` sample defines two user-defined data types, `CADatetime` and `CADate`, which provide calendar-aware handling of dates and times.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="17c9e-104">先决条件</span><span class="sxs-lookup"><span data-stu-id="17c9e-104">Prerequisites</span></span>  
 <span data-ttu-id="17c9e-105">若要创建和运行此项目，必须安装下列软件：</span><span class="sxs-lookup"><span data-stu-id="17c9e-105">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="17c9e-106">或 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express。</span><span class="sxs-lookup"><span data-stu-id="17c9e-106">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="17c9e-107">可以从 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express 文档和示例[网站](https://www.microsoft.com/sql-server/sql-server-editions-express)免费获取 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express</span><span class="sxs-lookup"><span data-stu-id="17c9e-107">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="17c9e-108">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]开发人员[网站](https://go.microsoft.com/fwlink/?linkid=62796)提供的 AdventureWorks 数据库</span><span class="sxs-lookup"><span data-stu-id="17c9e-108">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="17c9e-109">.NET Framework SDK 2.0 或更高版本，或 Microsoft Visual Studio 2005 或更高版本。</span><span class="sxs-lookup"><span data-stu-id="17c9e-109">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="17c9e-110">您可以免费获取 .NET Framework SDK。</span><span class="sxs-lookup"><span data-stu-id="17c9e-110">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="17c9e-111">此外，还必须满足以下条件：</span><span class="sxs-lookup"><span data-stu-id="17c9e-111">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="17c9e-112">使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例必须已启用 CLR 集成。</span><span class="sxs-lookup"><span data-stu-id="17c9e-112">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="17c9e-113">若要启用 CLR 集成，请执行以下步骤：</span><span class="sxs-lookup"><span data-stu-id="17c9e-113">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="17c9e-114">启用 CLR 集成</span><span class="sxs-lookup"><span data-stu-id="17c9e-114">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="17c9e-115">执行以下 [!INCLUDE[tsql](../../includes/tsql-md.md)] 命令：</span><span class="sxs-lookup"><span data-stu-id="17c9e-115">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="17c9e-116">若要启用 CLR，您必须具有 `ALTER SETTINGS` 服务器级别权限，`sysadmin` 和 `serveradmin` 固定服务器角色的成员隐式拥有该权限。</span><span class="sxs-lookup"><span data-stu-id="17c9e-116">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="17c9e-117">必须在您使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例上安装 AdventureWorks 数据库。</span><span class="sxs-lookup"><span data-stu-id="17c9e-117">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="17c9e-118">如果你不是要使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例的管理员，必须请管理员授予你 **CreateAssembly** 权限，才能完成安装。</span><span class="sxs-lookup"><span data-stu-id="17c9e-118">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="17c9e-119">生成示例</span><span class="sxs-lookup"><span data-stu-id="17c9e-119">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="17c9e-120">按照以下说明创建和运行该示例：</span><span class="sxs-lookup"><span data-stu-id="17c9e-120">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="17c9e-121">打开 Visual Studio 或 .NET Framework 命令提示符。</span><span class="sxs-lookup"><span data-stu-id="17c9e-121">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="17c9e-122">如有必要，为您的示例创建目录。</span><span class="sxs-lookup"><span data-stu-id="17c9e-122">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="17c9e-123">对于此示例，我们将使用 C:\MySample。</span><span class="sxs-lookup"><span data-stu-id="17c9e-123">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="17c9e-124">在 c:\MySample 中，创建 `CalendarAware.cs` 并将 C# 示例代码（如下所示）复制到该文件中。</span><span class="sxs-lookup"><span data-stu-id="17c9e-124">In c:\MySample, create `CalendarAware.cs` and copy the C# sample code (below) into the file.</span></span>  
  
4.  <span data-ttu-id="17c9e-125">在 c:\MySample 中，创建文件 `calendars.txt` 并将示例代码复制到该文件中。</span><span class="sxs-lookup"><span data-stu-id="17c9e-125">In c:\MySample, create the file `calendars.txt` and copy the sample code into the file.</span></span>  
  
5.  <span data-ttu-id="17c9e-126">在 c:\MySample 中，创建文件 `calendars.ar-SA.txt` 并将以下代码复制到该文件中：</span><span class="sxs-lookup"><span data-stu-id="17c9e-126">In c:\MySample, create the file `calendars.ar-SA.txt` and copy the following into the file:</span></span>  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 1`  
  
6.  <span data-ttu-id="17c9e-127">在 c:\MySample 中，创建文件 `calendars.ja.txt` 并将以下代码复制到该文件中：</span><span class="sxs-lookup"><span data-stu-id="17c9e-127">In c:\MySample, create the file `calendars.ja.txt` and copy the following into the file:</span></span>  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 3`  
  
7.  <span data-ttu-id="17c9e-128">在 c:\MySample 中，创建文件 `calendars.``zh-CN.txt` 并将以下代码复制到该文件中：</span><span class="sxs-lookup"><span data-stu-id="17c9e-128">In c:\MySample, create the file `calendars.``zh-CN.txt` and copy the following into the file:</span></span>  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 8`  
  
8.  <span data-ttu-id="17c9e-129">在 c:\MySample 中，创建文件 `build.com` 并将以下代码复制到该文件中：</span><span class="sxs-lookup"><span data-stu-id="17c9e-129">In c:\MySample, create the file `build.com` and copy the following into the file:</span></span>  
...
9. <span data-ttu-id="17c9e-130">通过在命令提示符处执行文件生成操作来生成附属程序集。</span><span class="sxs-lookup"><span data-stu-id="17c9e-130">Build the satellite assembles by executing the file build at the command prompt.</span></span>  
  
10. <span data-ttu-id="17c9e-131">通过在命令行提示符处执行以下命令来编译示例代码：</span><span class="sxs-lookup"><span data-stu-id="17c9e-131">Compile the sample code from the command line prompt by executing the following:</span></span>  
...
11. <span data-ttu-id="17c9e-132">将 [!INCLUDE[tsql](../../includes/tsql-md.md)] 安装代码复制到一个文件中，并在示例目录中将其另存为 `Install.sql`。</span><span class="sxs-lookup"><span data-stu-id="17c9e-132">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
12. <span data-ttu-id="17c9e-133">如果示例安装在 `C:\MySample\` 之外的目录中，请按说明编辑文件 `Install.sql` 以指向该位置。</span><span class="sxs-lookup"><span data-stu-id="17c9e-133">If the sample is installed in a directory other then `C:\MySample\`, edit the file `Install.sql` as indicated to point to that location.</span></span>  
  
13. <span data-ttu-id="17c9e-134">通过执行以下命令部署程序集和存储过程：</span><span class="sxs-lookup"><span data-stu-id="17c9e-134">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql`  
  
14. <span data-ttu-id="17c9e-135">将 [!INCLUDE[tsql](../../includes/tsql-md.md)] 测试命令脚本复制到文件中，并将其另存为 `test.sql` 示例目录中的。</span><span class="sxs-lookup"><span data-stu-id="17c9e-135">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
15. <span data-ttu-id="17c9e-136">使用以下命令执行测试脚本：</span><span class="sxs-lookup"><span data-stu-id="17c9e-136">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
16. <span data-ttu-id="17c9e-137">将 [!INCLUDE[tsql](../../includes/tsql-md.md)] 清除脚本复制到一个文件中，并在示例目录中将其另存为 `cleanup.sql`。</span><span class="sxs-lookup"><span data-stu-id="17c9e-137">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
17. <span data-ttu-id="17c9e-138">使用以下命令执行该脚本：</span><span class="sxs-lookup"><span data-stu-id="17c9e-138">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
## <a name="sample-code"></a><span data-ttu-id="17c9e-139">代码示例</span><span class="sxs-lookup"><span data-stu-id="17c9e-139">Sample Code</span></span>  
 <span data-ttu-id="17c9e-140">下面是此示例的代码列表。</span><span class="sxs-lookup"><span data-stu-id="17c9e-140">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="17c9e-141">C#</span><span class="sxs-lookup"><span data-stu-id="17c9e-141">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="17c9e-142">这是定义日历所需的文件 `calendars.txt`。</span><span class="sxs-lookup"><span data-stu-id="17c9e-142">This is the file `calendars.txt` needed for defining the calendars.</span></span>  
...
```  
 ...
 <span data-ttu-id="17c9e-143">这是 [!INCLUDE[tsql](../../includes/tsql-md.md)] 安装脚本 (`Install.sql`)，该脚本在数据库中部署程序集并创建转换函数；它还创建了示例数据表。</span><span class="sxs-lookup"><span data-stu-id="17c9e-143">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assemblies and creates the conversion functions within the database; it also crates a table of sample data.</span></span>  
...
```  
 ...
 <span data-ttu-id="17c9e-144">这是 `test.sql`，该脚本通过对多个日历执行函数来测试该示例。</span><span class="sxs-lookup"><span data-stu-id="17c9e-144">This is `test.sql`, which tests the sample by executing the functions on various calendars.</span></span>  
...
```  
 ...
 <span data-ttu-id="17c9e-145">下面的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 从数据库中删除程序集、函数和表。</span><span class="sxs-lookup"><span data-stu-id="17c9e-145">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assemblies, functions and table from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="17c9e-146">另请参阅</span><span class="sxs-lookup"><span data-stu-id="17c9e-146">See Also</span></span>  
 [<span data-ttu-id="17c9e-147">公共语言运行时 (CLR) 集成的使用方案和示例</span><span class="sxs-lookup"><span data-stu-id="17c9e-147">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
