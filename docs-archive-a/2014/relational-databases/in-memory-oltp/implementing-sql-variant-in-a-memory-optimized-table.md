...
ms.locfileid: "87576749"
---
# <a name="implementing-sql_variant-in-a-memory-optimized-table"></a><span data-ttu-id="909fb-102">在内存优化的表中实现 SQL_VARIANT</span><span class="sxs-lookup"><span data-stu-id="909fb-102">Implementing SQL_VARIANT in a Memory-Optimized Table</span></span>
  <span data-ttu-id="909fb-103">考虑具有 `SQL_VARIANT` 列的表的示例：</span><span class="sxs-lookup"><span data-stu-id="909fb-103">Consider an example of a table with `SQL_VARIANT` column:</span></span>  
  
```sql  
CREATE TABLE [dbo].[T1]([Key] [sql_variant] NOT NULL)  
```  
  
 <span data-ttu-id="909fb-104">假定键列只能是 `BIGINT` 或 `NVARCHAR(300)`。</span><span class="sxs-lookup"><span data-stu-id="909fb-104">Assume that the key column can only be either a `BIGINT` or `NVARCHAR(300)`.</span></span> <span data-ttu-id="909fb-105">您可以按以下方式对此表建模：</span><span class="sxs-lookup"><span data-stu-id="909fb-105">You can model this table as follows:</span></span>  
...
```  
 ...
 <span data-ttu-id="909fb-106">现在您可以通过在 T1 上打开游标，将数据加载到 T1 的 [T1_HK]：</span><span class="sxs-lookup"><span data-stu-id="909fb-106">Now you can load data into [T1_HK] from T1 by opening a cursor on T1:</span></span>  
...
```  
 ...
 <span data-ttu-id="909fb-107">您可以按以下方式将数据转换回 `SQL_VARIANT`：</span><span class="sxs-lookup"><span data-stu-id="909fb-107">You can convert data back to `SQL_VARIANT` as follows:</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="909fb-108">另请参阅</span><span class="sxs-lookup"><span data-stu-id="909fb-108">See Also</span></span>  
 [<span data-ttu-id="909fb-109">迁移到内存中 OLTP</span><span class="sxs-lookup"><span data-stu-id="909fb-109">Migrating to In-Memory OLTP</span></span>](migrating-to-in-memory-oltp.md)  
