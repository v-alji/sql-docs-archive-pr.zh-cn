...
11. 在 "**选择空间数据和地图视图选项**" 页上，确认 "**空间" 字段**为 `SpatialLocation` ，"**层类型**" 为 "**点**"。 接受本页上的其他默认值。  
...
17. 在 "**数据" 字段**中，选择此项可根据 `[Sum(SellingArea)]` 商店设置的区域的大小改变标记类型以显示产品。  
...
7.  再次单击 "**标题**" 以进入文本插入模式。 按销售额将**标题**替换** (千) **，然后在文本外部单击。  
