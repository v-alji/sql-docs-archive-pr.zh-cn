...
# <a name="expression-uses-in-reports-report-builder-and-ssrs"></a>在报表中使用表达式（报表生成器和 SSRS）
  可以在整个报表定义中使用表达式来指定或计算以下各项的值：参数、查询、筛选器、报表项属性、分组和排序定义、文本框属性、书签、文档结构图、动态页眉和页脚内容、图像以及动态数据源定义。 本主题提供了一些可在其中使用表达式更改报表内容或外观的示例。 此列表并不全面。 可以在对话框中为任何属性设置表达式，该对话框显示的表达式 (**fx**) 按钮或显示的下拉列表 **\<Expression...>** 。  
...
|隐藏设计图面上的文本框，用户可使用名为 *Show*的布尔型参数切换该文本框。|文本框中的 Hidden 属性。 使用 **“文本框属性”** 对话框 -&gt;“可见性”。|`=Not Parameters!`*显示 \<boolean parameter> *`.Value`|  
