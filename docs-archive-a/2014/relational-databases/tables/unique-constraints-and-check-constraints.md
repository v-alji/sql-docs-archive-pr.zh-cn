...
### <a name="limitations-of-check-constraints"></a>CHECK 约束的限制  
 CHECK 约束不接受计算结果为 FALSE 的值。 因为空值的计算结果为 UNKNOWN，所以表达式中存在这些值可能会覆盖约束。 例如，假设您对列设置了约束， `int` **MyColumn**指定**MyColumn**只能包含值 10 (**MyColumn = 10**) 。 如果将值 NULL 插入到 **MyColumn**， [!INCLUDE[ssDE](../../includes/ssde-md.md)] 将插入 NULL 且不返回错误。  
