...
5.  调用 SQLExecute 或 SQLExecDirect。  
 ...
 如果将*StrLen_or_IndPtr*设置为 SQL_LEN_DATA_AT_EXEC (*长度*) 或列 SQL_DATA_AT_EXEC，则输入表值参数列的值可以进行传入。 这类似于使用参数数组时的分块传递值。 与所有执行时数据参数一样，SQLParamData 不指示驱动程序要向哪个数组行请求数据;应用程序必须对此进行处理。 应用程序不能对驱动程序将请求值的顺序做出任何假设。  
