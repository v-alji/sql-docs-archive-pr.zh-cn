...
## <a name="command-options-for-unicode-character-format"></a>Unicode 字符格式的命令选项  
 可以使用**bcp**、BULK INSERT 或 INSERT ... 将 Unicode 字符格式数据导入表中。\*从 OPENROWSET (BULK ... ) 中进行选择。对于**bcp**命令或 BULK INSERT 语句，可以在命令行中指定数据格式。 对于 INSERT ...SELECT * FROM OPENROWSET(BULK...) 语句，必须在格式化文件中指定数据格式。  
...
|----------------|-----------------|  
|**-w**|指定 Unicode 字符格式。|  
|**-t** `,`|将逗号 (`,`) 指定为字段终止符。<br /><br /> 注意：默认的字段终止符是 ( \t) 的 tab 键。 有关详细信息，请参阅 [指定字段终止符和行终止符 (SQL Server)](specify-field-and-row-terminators-sql-server.md)。|  
