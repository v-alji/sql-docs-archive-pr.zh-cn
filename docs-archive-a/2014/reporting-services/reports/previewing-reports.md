...
> [!NOTE]  
> 在预览报表时，报表数据将缓存到本地计算机上的文件中。 使用相同的查询、参数和凭据再次预览同一报表时，报表设计器将检索缓存副本，而不是重新运行查询。 数据文件 *\<reportname>* 在报表定义文件所在的同一目录中另存为 .rdl。 关闭报表设计器时，不会删除该文件。  
...
- 单击 "标准" 工具栏上的 "**开始**" 按钮 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 。  
...
> [!NOTE]  
> 在调试报表之前，必须设置开始项。 若要设置启动项，请在解决方案资源管理器中右键单击报表项目，再单击 "**属性**"，然后在中 `StartItem` ，选择要显示的报表的名称。  
