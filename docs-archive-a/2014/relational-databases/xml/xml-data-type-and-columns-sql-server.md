...
-   本机存储，数据类型为 `xml`  
     ...
     对于类型化的 `xml` 数据类型、 `xml` 绑定到 XML 架构的数据类型， () PSVI 的后架构验证信息集会将类型信息添加到信息集并在内部表示形式进行编码。 这会显著提高分析速度。 有关详细信息，请参阅位于 [http://www.w3.org/TR/xmlschema-1](https://go.microsoft.com/fwlink/?LinkId=48881) 和 [http://www.w3.org/TR/xmlschema-2](https://go.microsoft.com/fwlink/?LinkId=4871) 的 W3C XML 架构规范。  
