...
```  
 ...
 在查询中， `child` 是轴， `Customer` 并且 `Order` 是节点测试 (如果客户和订单是节点，则这些节点测试是否为 TRUE **\<element>** ，因为 **\<element>** 节点是轴的主节点 `child`) 。 对于每个匹配的节点 **\<Customer>** ，会将匹配的节点 **\<Orders>** 添加到结果中。 仅 **\<Order>** 在结果集中返回。  
