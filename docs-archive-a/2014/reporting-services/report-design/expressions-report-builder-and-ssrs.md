...
表达式以等号开头 (=) 并写入 [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[vbprvb](../../../includes/vbprvb-md.md)] 。 表达式可包含以下项的组合：常量、运算符、对内置值（字段、集合和函数）的引用以及对外部或自定义代码的引用。  
...
-   **复杂** ：复杂表达式包含对多个内置引用、运算符和函数调用的引用。 \<Expr>当表达式值包括多个简单引用时，复杂表达式显示为 <>。 若要查看表达式，请将鼠标指针悬停在表达式上并使用工具提示。 若要编辑表达式，请在 **“表达式”** 对话框中打开它。  
