...
5.  执行 VBScript 代码。  
     ...
     `sql:datatype`当**customerid**的值指定为包含大括号 ( {and} ) 的 GUID 时，架构必须为**customerid**属性指定对应的，例如：  
    ...
    ```  
     ...
     如果 `sql:datatype` 指定将列类型标识为 `uniqueidentifier` ，则大容量加载操作将从**CustomerID**值中删除大括号 ( {and} ) ，然后将其插入列中。  
