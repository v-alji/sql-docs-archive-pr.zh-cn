...
-   要大容量加载到类型为的列 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] `uniqueidentifier` ，而 XSD 值是包含大括号 ( {and} ) 的 GUID。 指定 `sql:datatype="uniqueidentifier"` 时，在将值插入列之前从值中删除大括号。 如果未指定 `sql:datatype`，将发送包含大括号的值并且插入或更新失败。  
