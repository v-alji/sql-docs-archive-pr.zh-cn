...
2.  在 "**表列**" 下，单击 `Bike Buyer` 单元格，然后从下拉列表中选择 "ProspectiveBuyer"。  
...
6.  在 "**创建命名计算**" 对话框中，键入 "**列名称**" `calcAge` 。  
  
7.  对于 "**说明**"，键入**基于生日计算年龄**。  
  
8.  在 "**表达式**" 框中键入， `DATEDIFF(YYYY,[BirthDate],getdate())` 然后单击 **"确定"**。  
...
6.  在行的 " `TM_Decision_Tree` **字段**" 列中，选择 `Bike Buyer` 。  
  
7.  在 `TM_Decision_Tree` 行的 "**条件/参数**" 列中，键入 `=1` 。  
  
8.  单击 "**源**" 列中的下一个空行，然后选择 " **ProspectiveBuyer 表**"。  
  
9. 在行的 " `ProspectiveBuyer` **字段**" 列中，选择 " **ProspectiveBuyerKey**"。  
...
3.  此时您可以保存结果。 您有三种选择：  
    ...
    -   右键单击结果中的行数据，然后选择 "**复制**"，只保存该值 (并且列标题) 到剪贴板。  
    ...
    -   单击 "**保存查询结果**" 将结果直接保存到数据库，如下所示：  
        ...
        4.  使用 "**如果存在，则覆盖**" 选项来更新具有最新结果的现有表。  
             ...
             此外，如果多个列具有相同的名称 (例如，默认的列名**表达式**) 您必须为具有重复名称的列创建一个别名，否则当设计器尝试将结果保存到 SQL Server 时将引发错误。 原因是 SQL Server 不允许多个列具有相同的名称。  
