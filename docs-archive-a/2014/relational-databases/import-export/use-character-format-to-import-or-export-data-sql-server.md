...
## <a name="command-options-for-character-format"></a>字符格式的命令选项  
 可以使用**bcp**、BULK INSERT 或 INSERT ... 将字符格式数据导入表中。\*从 OPENROWSET (BULK ... ) 中进行选择。对于**bcp**命令或 BULK INSERT 语句，可以在命令行中指定数据格式。 对于 INSERT ...SELECT * FROM OPENROWSET(BULK...) 语句，必须在格式化文件中指定数据格式。  
