...
ms.locfileid: "87694094"
---
# <a name="handling-large-objects-using-clr"></a><span data-ttu-id="a025b-102">使用 CLR 处理大型对象</span><span class="sxs-lookup"><span data-stu-id="a025b-102">Handling Large Objects Using CLR</span></span>
  <span data-ttu-id="a025b-103">SQL Server 的 `HandlingLOBUsingCLR` 示例说明了如何通过使用公共语言运行时 (CLR) 存储过程在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 与服务器可访问的文件系统之间传输大型对象 (LOB)。</span><span class="sxs-lookup"><span data-stu-id="a025b-103">The `HandlingLOBUsingCLR` sample for SQL Server demonstrates transferring large objects (LOBs) between [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] and a file system that is available to the server by using common language runtime (CLR) stored procedures.</span></span> <span data-ttu-id="a025b-104">本示例说明了如何在服务器端代码中访问文件，以及如何从基于 CLR 的存储过程中调用动态查询和存储过程。</span><span class="sxs-lookup"><span data-stu-id="a025b-104">This sample demonstrates how to access files in server-side code, and then invoke both dynamic queries and stored procedures from CLR-based stored procedures.</span></span> <span data-ttu-id="a025b-105">此示例还说明了如何使用 [!INCLUDE[tsql](../../includes/tsql-md.md)] 注册和注销 CLR 方法和程序集。</span><span class="sxs-lookup"><span data-stu-id="a025b-105">It also demonstrates how to unregister and register CLR methods and assemblies by using [!INCLUDE[tsql](../../includes/tsql-md.md)].</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="a025b-106">先决条件</span><span class="sxs-lookup"><span data-stu-id="a025b-106">Prerequisites</span></span>  
 <span data-ttu-id="a025b-107">若要创建和运行此项目，必须安装下列软件：</span><span class="sxs-lookup"><span data-stu-id="a025b-107">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="a025b-108">或 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express。</span><span class="sxs-lookup"><span data-stu-id="a025b-108">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="a025b-109">可以从 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express 文档和示例[网站](https://www.microsoft.com/download/details.aspx?id=42299)免费获取 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express</span><span class="sxs-lookup"><span data-stu-id="a025b-109">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/download/details.aspx?id=42299)</span></span>  
  
-   <span data-ttu-id="a025b-110">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]开发人员[网站](https://archive.codeplex.com/?p=SqlServerSamples)提供的 AdventureWorks 数据库</span><span class="sxs-lookup"><span data-stu-id="a025b-110">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://archive.codeplex.com/?p=SqlServerSamples)</span></span>  
  
-   <span data-ttu-id="a025b-111">.NET Framework SDK 2.0 或更高版本，或 Microsoft Visual Studio 2005 或更高版本。</span><span class="sxs-lookup"><span data-stu-id="a025b-111">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="a025b-112">您可以免费获取 .NET Framework SDK。</span><span class="sxs-lookup"><span data-stu-id="a025b-112">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="a025b-113">此外，还必须满足以下条件：</span><span class="sxs-lookup"><span data-stu-id="a025b-113">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="a025b-114">使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例必须已启用 CLR 集成。</span><span class="sxs-lookup"><span data-stu-id="a025b-114">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="a025b-115">若要启用 CLR 集成，请执行以下步骤：</span><span class="sxs-lookup"><span data-stu-id="a025b-115">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="a025b-116">启用 CLR 集成</span><span class="sxs-lookup"><span data-stu-id="a025b-116">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="a025b-117">执行以下 [!INCLUDE[tsql](../../includes/tsql-md.md)] 命令：</span><span class="sxs-lookup"><span data-stu-id="a025b-117">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="a025b-118">若要启用 CLR，您必须具有 `ALTER SETTINGS` 服务器级别权限，`sysadmin` 和 `serveradmin` 固定服务器角色的成员隐式拥有该权限。</span><span class="sxs-lookup"><span data-stu-id="a025b-118">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="a025b-119">必须在您使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例上安装 AdventureWorks 数据库。</span><span class="sxs-lookup"><span data-stu-id="a025b-119">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="a025b-120">如果你不是要使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例的管理员，必须请管理员授予你 **CreateAssembly** 权限，才能完成安装。</span><span class="sxs-lookup"><span data-stu-id="a025b-120">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="a025b-121">生成示例</span><span class="sxs-lookup"><span data-stu-id="a025b-121">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="a025b-122">按照以下说明创建和运行该示例：</span><span class="sxs-lookup"><span data-stu-id="a025b-122">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="a025b-123">打开 Visual Studio 或 .NET Framework 命令提示符。</span><span class="sxs-lookup"><span data-stu-id="a025b-123">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="a025b-124">如有必要，为您的示例创建目录。</span><span class="sxs-lookup"><span data-stu-id="a025b-124">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="a025b-125">对于此示例，我们将使用 C:\MySample。</span><span class="sxs-lookup"><span data-stu-id="a025b-125">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="a025b-126">由于此示例需要签名的程序集，请通过键入以下命令创建一个非对称密钥：</span><span class="sxs-lookup"><span data-stu-id="a025b-126">Since this example requires a signed assembly, create an asymmetric key by typing the command:</span></span>  
  
## <a name="sample-code"></a><span data-ttu-id="a025b-127">代码示例</span><span class="sxs-lookup"><span data-stu-id="a025b-127">Sample Code</span></span>  
 <span data-ttu-id="a025b-128">下面是此示例的代码列表。</span><span class="sxs-lookup"><span data-stu-id="a025b-128">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="a025b-129">C#</span><span class="sxs-lookup"><span data-stu-id="a025b-129">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="a025b-130">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="a025b-130">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="a025b-131">这是 [!INCLUDE[tsql](../../includes/tsql-md.md)] 安装脚本 (`Install.sql`)，该脚本部署程序集并创建此示例所需的存储过程和安全对象。</span><span class="sxs-lookup"><span data-stu-id="a025b-131">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the stored procedures and security objects required by this example.</span></span>  
...
```  
 ...
 <span data-ttu-id="a025b-132">以下 [!INCLUDE[tsql](../../includes/tsql-md.md)] (`test.sql` 通过执行存储过程) 测试示例。</span><span class="sxs-lookup"><span data-stu-id="a025b-132">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] (`test.sql`) tests the example by exercising the stored procedures.</span></span>  
...
```  
 ...
 <span data-ttu-id="a025b-133">下面的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 从数据库中删除程序集、登录名、密钥和存储过程。</span><span class="sxs-lookup"><span data-stu-id="a025b-133">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly, login, key and stored procedures from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="a025b-134">另请参阅</span><span class="sxs-lookup"><span data-stu-id="a025b-134">See Also</span></span>  
 [<span data-ttu-id="a025b-135">公共语言运行时 (CLR) 集成的使用方案和示例</span><span class="sxs-lookup"><span data-stu-id="a025b-135">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
