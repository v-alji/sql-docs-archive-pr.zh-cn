...
10. 单击 "**受信任的文件位置**"，并在此页上选择你的可信位置。  (通常，此列在 Address 列中作为**http://** 列出。 ) 若要确保 Excel Services 和 PowerPivot 服务都有权访问该工作簿，您必须将 SharePoint 作为 Excel services 受信任位置包括在内。 PowerPivot 系统服务无法访问在 SharePoint 场的外部存储的工作簿。  
