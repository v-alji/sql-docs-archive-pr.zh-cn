...
### <a name="connecting-to-a-shared-instance-of-localdb"></a>连接到 LocalDB 的共享实例  
 若要连接到 `LocalDB` ** \\ ** add 的共享实例，请将 (点 + 反斜杠) 到连接字符串，以便引用为共享实例保留的命名空间。 例如，若要连接到名为的共享实例，请 `LocalDB` `AppData` 使用连接字符串（例如） `(localdb)\.\AppData` 作为连接字符串的一部分。 连接到其不拥有的共享实例的用户 `LocalDB` 必须具有 Windows 身份验证或 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 身份验证登录名。  
