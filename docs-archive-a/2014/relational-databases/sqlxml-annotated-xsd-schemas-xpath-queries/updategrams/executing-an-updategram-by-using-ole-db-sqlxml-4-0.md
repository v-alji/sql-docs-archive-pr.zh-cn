...
ms.locfileid: "87689801"
---
# <a name="executing-an-updategram-by-using-ole-db-sqlxml-40"></a><span data-ttu-id="89572-102">使用 OLE DB 执行 Updategram (SQLXML 4.0)</span><span class="sxs-lookup"><span data-stu-id="89572-102">Executing an Updategram by Using OLE DB (SQLXML 4.0)</span></span>
  <span data-ttu-id="89572-103">本主题提供了 usingOLE DB 的工作示例，以执行 updategram。</span><span class="sxs-lookup"><span data-stu-id="89572-103">This topic provides a working sample of usingOLE DB to execute an updategram.</span></span>  
  
## <a name="using-icommandstream-to-set-an-xml-command"></a><span data-ttu-id="89572-104">使用 ICommandStream 设置 XML 命令</span><span class="sxs-lookup"><span data-stu-id="89572-104">Using ICommandStream to Set an XML Command</span></span>  
 <span data-ttu-id="89572-105">OLE DB (版本2.6 或更高版本) ICommandStream 接口将命令作为流对象传递，而不是作为字符串传递。</span><span class="sxs-lookup"><span data-stu-id="89572-105">The OLE DB (version 2.6 or later) ICommandStream interface passes a command as a stream object, rather than as a string.</span></span>  
  
 <span data-ttu-id="89572-106">该接口允许命令采用 XML 分析器理解的任何编码形式。</span><span class="sxs-lookup"><span data-stu-id="89572-106">This interface allows a command to be in any encoding that the XML parser understands.</span></span> <span data-ttu-id="89572-107">调用 ICommand：： Execute 时，将直接从流中读取命令文本，而不需要进行任何转换。</span><span class="sxs-lookup"><span data-stu-id="89572-107">When ICommand::Execute is called, the command text is read out of the stream directly and no conversion is required.</span></span> <span data-ttu-id="89572-108">因此，使用 ICommandStream 接口执行 XML 命令更有效。</span><span class="sxs-lookup"><span data-stu-id="89572-108">Therefore, executing XML commands using ICommandStream interface is more efficient.</span></span>  
  
### <a name="setting-xml-as-a-command-using-icommandstream-and-retrieving-the-results-as-an-xml-document"></a><span data-ttu-id="89572-109">使用 IcommandStream 将 XML 设置为命令并以 XML 文档形式检索结果</span><span class="sxs-lookup"><span data-stu-id="89572-109">Setting XML as a Command Using ICommandStream and Retrieving the Results as an XML Document</span></span>  
 <span data-ttu-id="89572-110">ICommandStream 接口可用于将 XML 文档设置为命令，可将结果作为 XML 文档来检索。</span><span class="sxs-lookup"><span data-stu-id="89572-110">The ICommandStream interface can be used to set XML documents as a command, and the results can be retrieved as an XML document.</span></span>  
  
#### <a name="executing-templates-with-xpath-queries"></a><span data-ttu-id="89572-111">使用 XPath 查询执行模板</span><span class="sxs-lookup"><span data-stu-id="89572-111">Executing Templates with XPath Queries</span></span>  
 <span data-ttu-id="89572-112">以下 XML 模板由 XPath 查询指定为使用 ICommandStream 的命令：</span><span class="sxs-lookup"><span data-stu-id="89572-112">The following XML template consisting of an XPath query is specified as a command using ICommandStream:</span></span>  
...
```  
 ...
 <span data-ttu-id="89572-113">根据以下映射架构执行该模板中的 XPath 查询：</span><span class="sxs-lookup"><span data-stu-id="89572-113">The XPath query in the template is executed against the following mapping schema:</span></span>  
...
```  
 ...
 <span data-ttu-id="89572-114">查询返回所有雇员元素。</span><span class="sxs-lookup"><span data-stu-id="89572-114">The query returns all of the employee elements.</span></span> <span data-ttu-id="89572-115">在默认映射中， **\<Person.Contact>** 元素映射到 AdventureWorks 数据库中的 Person 表。</span><span class="sxs-lookup"><span data-stu-id="89572-115">With default mapping, the **\<Person.Contact>** element maps to the Person.Contact table in the AdventureWorks database.</span></span>  
  
###### <a name="to-set-xml-as-a-command-and-retrieving-result-as-an-xml-document"></a><span data-ttu-id="89572-116">将 XML 设置为命令并以 XML 文档形式检索结果</span><span class="sxs-lookup"><span data-stu-id="89572-116">To set XML as a command and retrieving result as an XML document</span></span>  
  
1.  <span data-ttu-id="89572-117">初始化并建立与数据库的连接。</span><span class="sxs-lookup"><span data-stu-id="89572-117">Initialize and establish a connection to the database.</span></span>  
  
2.  <span data-ttu-id="89572-118">获取 ICommand 上的 ICommandStream 接口。</span><span class="sxs-lookup"><span data-stu-id="89572-118">Obtain the ICommandStream interface on ICommand.</span></span>  
  
3.  <span data-ttu-id="89572-119">设置必要的命令属性。</span><span class="sxs-lookup"><span data-stu-id="89572-119">Set the necessary command properties.</span></span> <span data-ttu-id="89572-120">在本示例中，将特定于访问接口的 SSPROP_STREAM_BASEPATH 属性设置为存储映射架构和模板文件的目录。</span><span class="sxs-lookup"><span data-stu-id="89572-120">In this example, the provider-specific property SSPROP_STREAM_BASEPATH is set to the directory where the mapping schema and the template files are stored.</span></span>  
  
4.  <span data-ttu-id="89572-121">使用 ICommandStream：： SetCommandStream 来指定命令流。</span><span class="sxs-lookup"><span data-stu-id="89572-121">Use ICommandStream::SetCommandStream to specify the command stream.</span></span> <span data-ttu-id="89572-122">在本示例中，执行的 XML 模板是从文件中读取的。</span><span class="sxs-lookup"><span data-stu-id="89572-122">In this example, the XML template being executed is read from a file.</span></span> <span data-ttu-id="89572-123">在希望执行大型 XML 模板时，此功能非常有用。</span><span class="sxs-lookup"><span data-stu-id="89572-123">This is useful when you want to execute large XML templates.</span></span>  
  
5.  <span data-ttu-id="89572-124">使用 ICommand：： Execute 执行 XML 命令，请求 IID_ISequentialStream 接口 ID。</span><span class="sxs-lookup"><span data-stu-id="89572-124">Execute the XML command using ICommand::Execute, requesting the IID_ISequentialStream interface ID.</span></span>  
  
6.  <span data-ttu-id="89572-125">处理结果。</span><span class="sxs-lookup"><span data-stu-id="89572-125">Process the result.</span></span> <span data-ttu-id="89572-126">在本示例中，从流读取的 XML 将在屏幕上显示。</span><span class="sxs-lookup"><span data-stu-id="89572-126">In this example, the XML read from the stream is displayed on the screen.</span></span>  
...
#### <a name="passing-parameters-to-templates"></a><span data-ttu-id="89572-127">将参数传递给模板</span><span class="sxs-lookup"><span data-stu-id="89572-127">Passing Parameters to Templates</span></span>  
 <span data-ttu-id="89572-128">本示例显示如何向 XML 命令传递参数值。</span><span class="sxs-lookup"><span data-stu-id="89572-128">This example shows how parameter values can be passed to XML commands.</span></span> <span data-ttu-id="89572-129">将该 XML 模板指定为命令：</span><span class="sxs-lookup"><span data-stu-id="89572-129">This XML template is specified as a command:</span></span>  
...
```  
 ...
 <span data-ttu-id="89572-130">该模板包含 SQL 查询。</span><span class="sxs-lookup"><span data-stu-id="89572-130">The template includes an SQL query.</span></span> <span data-ttu-id="89572-131">查询需要 () 的参数的值 @Title 。</span><span class="sxs-lookup"><span data-stu-id="89572-131">The query requires a value for its parameter (@Title).</span></span> <span data-ttu-id="89572-132">如果未传递参数值，则使用默认值（“Mr.”）。</span><span class="sxs-lookup"><span data-stu-id="89572-132">If no parameter value is passed, the default value ("Mr.") is used.</span></span>  
  
 <span data-ttu-id="89572-133">向模板传递参数值时，参数名称和值都必须指定。</span><span class="sxs-lookup"><span data-stu-id="89572-133">In passing parameter values to a template, the parameter name and value both must be specified.</span></span>  
  
 <span data-ttu-id="89572-134">以下是相应代码：</span><span class="sxs-lookup"><span data-stu-id="89572-134">This is the code:</span></span>  
