...
## <a name="remarks"></a>备注  
 **dbm_monitor** 固定数据库角色成员可以使用数据库镜像监视器或 **sp_dbmmonitorresults** 存储过程查看现有的镜像状态。 但是这些用户不能更新状态表。 它们依赖于“数据库镜像监视器作业” 来定期更新状态表。 若要了解所显示的状态的保留时间，用户可以查看**主体日志 (的时间 ***\<time>***) **和**镜像日志 (***\<time>***) **标签。  
