...
##  <a name="missing-keys-keynotfound"></a><a name="bkmk_missing"></a>缺少密钥 (KeyNotFound)   
...
4.  将值更改为 `Error` ，以排除任何具有 null 值的记录，避免 (零到数字的) 转换。 使用此修改可以避免与键列中具有零的多个记录相关的重复键错误，也可避免 `KeyNotFound` 当零值外键在相关维度表中没有主键等效项时出现错误。  
