...
-   已通过使用 "[使用 Azure Blob 存储服务进行备份和还原 SQL Server](backup-restore/sql-server-backup-and-restore-with-microsoft-azure-blob-storage-service.md) " 功能在 azure 存储中创建数据库 ( .bak) 的备份。 注意，需要在此步骤中创建另一个 SQL Server 凭据。 此凭据使用存储帐户密钥。  
...
3.  复制以下完整脚本并将其粘贴到查询窗口。 根据需要修改脚本。  
     ...
     **注意：** 运行 `RESTORE` 语句，将 Azure 存储中的数据库备份 ( .bak) 还原到另一台计算机中的数据库实例。  
