...
# <a name="mapping-clr-parameter-data"></a>映射 CLR 参数数据
  下表列出了 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 数据类型、它们在公共语言运行时中的等效项 (CLR) [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 在 `System.Data.SqlTypes` 命名空间中，以及它们在 .NET FRAMEWORK 中的本机 clr 等效项 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 。  
