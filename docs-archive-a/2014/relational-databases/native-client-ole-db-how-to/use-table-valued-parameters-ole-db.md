...
ms.locfileid: "87692894"
---
# <a name="use-table-valued-parameters-ole-db"></a><span data-ttu-id="e42f4-102">使用表值参数 (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="e42f4-102">Use Table-Valued Parameters (OLE DB)</span></span>
  <span data-ttu-id="e42f4-103">此示例适用于 [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 或更高版本。</span><span class="sxs-lookup"><span data-stu-id="e42f4-103">This sample works with [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] or later.</span></span> <span data-ttu-id="e42f4-104">该示例将执行以下操作：</span><span class="sxs-lookup"><span data-stu-id="e42f4-104">This sample does the following:</span></span>  
  
-   <span data-ttu-id="e42f4-105">通过 IOpenRowset::OpenRowset 使用动态发现创建表值参数。</span><span class="sxs-lookup"><span data-stu-id="e42f4-105">Creates table-valued parameters by using dynamic discovery though IOpenRowset::OpenRowset.</span></span>  
  
-   <span data-ttu-id="e42f4-106">通过在 EmployeesRowset 类中使用请求模型发送表值参数行。</span><span class="sxs-lookup"><span data-stu-id="e42f4-106">Sends table-valued parameter rows by using the pull model in the EmployeesRowset class.</span></span> <span data-ttu-id="e42f4-107">在请求模型中，使用者按需为访问接口提供数据。</span><span class="sxs-lookup"><span data-stu-id="e42f4-107">In the pull model, the consumer provides data on demand to the provider.</span></span>  
  
-   <span data-ttu-id="e42f4-108">在 CPhotograph 类中发送作为表值参数一部分的 BLOB。</span><span class="sxs-lookup"><span data-stu-id="e42f4-108">Sends BLOBs as part of a table-valued parameter in the CPhotograph class.</span></span>  
  
-   <span data-ttu-id="e42f4-109">使用采用 ISSCommandWithParameters 的自定义参数属性。</span><span class="sxs-lookup"><span data-stu-id="e42f4-109">Uses custom parameter properties using ISSCommandWithParameters.</span></span>  
  
-   <span data-ttu-id="e42f4-110">显示 SQLNCLI11 错误的错误处理。</span><span class="sxs-lookup"><span data-stu-id="e42f4-110">Shows error handling for SQLNCLI11 errors.</span></span>  
  
 <span data-ttu-id="e42f4-111">有关表值参数的详细信息，请参阅[表值参数 &#40;SQL Server Native Client&#41;](../native-client/features/table-valued-parameters-sql-server-native-client.md)。</span><span class="sxs-lookup"><span data-stu-id="e42f4-111">For more information about table-valued parameters, see [Table-Valued Parameters &#40;SQL Server Native Client&#41;](../native-client/features/table-valued-parameters-sql-server-native-client.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="e42f4-112">示例</span><span class="sxs-lookup"><span data-stu-id="e42f4-112">Example</span></span>  
 <span data-ttu-id="e42f4-113">第一个 ([!INCLUDE[tsql](../../includes/tsql-md.md)]) 代码列表创建该示例使用的数据库。</span><span class="sxs-lookup"><span data-stu-id="e42f4-113">The first ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing creates the database used by the sample.</span></span>  
  
 <span data-ttu-id="e42f4-114">将第二个代码列表放入名为 stdafx.h 的文件。</span><span class="sxs-lookup"><span data-stu-id="e42f4-114">Put the second code listing into a file called stdafx.h.</span></span>  
  
 <span data-ttu-id="e42f4-115">将第三个代码列表放入名为 OLEDBUtils.hpp 的文件。</span><span class="sxs-lookup"><span data-stu-id="e42f4-115">Put the third code listing into a file called OLEDBUtils.hpp.</span></span>  
  
 <span data-ttu-id="e42f4-116">使用 ole32.lib 和 oleaut32.lib 编译并执行第四个 (C++) 代码列表。</span><span class="sxs-lookup"><span data-stu-id="e42f4-116">Compile with ole32.lib oleaut32.lib and execute the fourth (C++) code listing.</span></span> <span data-ttu-id="e42f4-117">此应用程序连接到您的计算机上默认的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例。</span><span class="sxs-lookup"><span data-stu-id="e42f4-117">This application connects to your computer's default [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="e42f4-118">在某些 Windows 操作系统上，您需要将 (localhost) 或 (local) 更改为您的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例的名称。</span><span class="sxs-lookup"><span data-stu-id="e42f4-118">On some Windows operating systems, you will need to change (localhost) or (local) to the name of your [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="e42f4-119">若要连接到命名实例，请将连接字符串从 L"(local)" 更改为 L"(local)\\\name"，其中 name 是命名实例。</span><span class="sxs-lookup"><span data-stu-id="e42f4-119">To connect to a named instance, change the connection string from L"(local)" to L"(local)\\\name" , where name is the named instance.</span></span> <span data-ttu-id="e42f4-120">默认情况下，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express 安装在命名实例中。</span><span class="sxs-lookup"><span data-stu-id="e42f4-120">By default, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express installs to a named instance.</span></span> <span data-ttu-id="e42f4-121">请确保 INCLUDE 环境变量包含包含 sqlncli.msi 的目录。</span><span class="sxs-lookup"><span data-stu-id="e42f4-121">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="e42f4-122">第五个 ([!INCLUDE[tsql](../../includes/tsql-md.md)]) 代码列表创建该示例使用的数据库。</span><span class="sxs-lookup"><span data-stu-id="e42f4-122">The fifth ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing creates the database used by the sample.</span></span>  
