...
-   用于安装 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的帐户应具有对 SMB 文件服务器的 SeSecurityPrivilege 特权。 若要授予此权限，请使用文件服务器上的 "本地安全策略" 控制台将 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 安装帐户添加到 "**管理审核和安全日志**" 策略中。 此设置可在 "**本地安全策略**" 控制台中 "**本地策略**" 下的 "**用户权限分配**" 部分中找到。  
