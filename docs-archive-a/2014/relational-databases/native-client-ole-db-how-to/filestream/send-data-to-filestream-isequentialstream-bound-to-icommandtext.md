---
title: 使用绑定到 ICommandText 参数的 ISequentialStream 将数据发送到 FILESTREAM 列 (OLE DB) |Microsoft Docs
ms.custom: ''
ms.date: 03/08/2017
...
ms.locfileid: "87581173"
---
# <a name="send-data-to-a-filestream-column-using-isequentialstream-bound-to-icommandtext-parameter-ole-db"></a><span data-ttu-id="069af-102">使用绑定到 ICommandText 参数的 ISequentialStream 向 FILESTREAM 列发送数据 (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="069af-102">Send Data to a FILESTREAM Column Using ISequentialStream Bound to ICommandText Parameter (OLE DB)</span></span>
  <span data-ttu-id="069af-103">此示例使用绑定到 ICommandText 参数的 ISequentialStream 接口向 filestream 列发送大小在 4MB 与 4GB 之间的数据。</span><span class="sxs-lookup"><span data-stu-id="069af-103">This sample uses an ISequentialStream interface bound to an ICommandText parameter to send between 4MB and 4GB of data to a filestream column.</span></span>  
  
 <span data-ttu-id="069af-104">有关 filestream 功能的详细信息，请参阅[Filestream 支持 &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md)。</span><span class="sxs-lookup"><span data-stu-id="069af-104">For more information on the filestream feature, see [FILESTREAM Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="069af-105">示例</span><span class="sxs-lookup"><span data-stu-id="069af-105">Example</span></span>  
 <span data-ttu-id="069af-106">编译和运行此示例前，先启用 FILESTREAM 支持（[启用和配置 FILESTREAM](../../blob/enable-and-configure-filestream.md)）。</span><span class="sxs-lookup"><span data-stu-id="069af-106">Before you compile and run this sample, enable FILESTREAM support ([Enable and Configure FILESTREAM](../../blob/enable-and-configure-filestream.md)).</span></span>  
  
 <span data-ttu-id="069af-107">请确保 INCLUDE 环境变量包含包含 sqlncli.msi 的目录。</span><span class="sxs-lookup"><span data-stu-id="069af-107">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="069af-108">该服务器必须具有名为 C:\DBFsa 的目录，示例将在其中创建数据库。</span><span class="sxs-lookup"><span data-stu-id="069af-108">The server must have a directory called C:\DBFsa; this is where the sample will create the database.</span></span> <span data-ttu-id="069af-109">您的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 实例必须对此位置具有写访问权限（例如，采用本地系统帐户登录）。</span><span class="sxs-lookup"><span data-stu-id="069af-109">Your instance of [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] must have write access to this location (for example, log on as a local system account).</span></span>  
  
 <span data-ttu-id="069af-110">复制第一个代码列表，并将其粘贴到名为 ISSHelper.h 的文件中。</span><span class="sxs-lookup"><span data-stu-id="069af-110">Copy the first code listing and paste it into a file called ISSHelper.h.</span></span>  
  
 <span data-ttu-id="069af-111">复制第二个代码列表，并将其粘贴到名为 ISSHelper.cpp 的文件中。</span><span class="sxs-lookup"><span data-stu-id="069af-111">Copy the second code listing and paste it into a file called ISSHelper.cpp.</span></span>  
  
 <span data-ttu-id="069af-112">复制第三个代码列表，并将其粘贴到名为 ICommandUpload.cpp 的文件中。</span><span class="sxs-lookup"><span data-stu-id="069af-112">Copy the third code listing and paste it into a file called ICommandUpload.cpp.</span></span>  
  
 <span data-ttu-id="069af-113">编译 ICommandUpload.cpp、ISSHelper.cpp、ole32.lib 和 oleaut32.lib。</span><span class="sxs-lookup"><span data-stu-id="069af-113">Compile ICommandUpload.cpp, ISSHelper.cpp, ole32.lib, and oleaut32.lib.</span></span>  
  
 <span data-ttu-id="069af-114">运行此示例时，必须传递服务器名称或“服务器\实例名称”，以及一个介于 4 MB (0x400001) 与 4 GB (0xFFFFFFFF) 之间的值，指示写入的数据量。</span><span class="sxs-lookup"><span data-stu-id="069af-114">When you run this sample, you must pass the name of a server, or a server\instance_name, as well as a value between 4 MB (0x400001) and 4 GB (0xFFFFFFFF) indicating the amount of data to write.</span></span>  
  
 <span data-ttu-id="069af-115">第四个 ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) 代码列表删除此示例创建的数据库。</span><span class="sxs-lookup"><span data-stu-id="069af-115">The fourth ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) code listing deletes the database created by this sample.</span></span>  
