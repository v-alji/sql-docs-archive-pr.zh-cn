...
4.  验证连接类型是否为 "Microsoft SQL Server"，然后在 "**连接字符串**" 框中键入： **Data \<servername> Source =**
     ...
     \<servername>例如，Report001) ，指定在其上安装 SQL Server 数据库引擎实例的计算机。 因为报表数据不是从 SQL Server 数据库提取的，所以不需要包括数据库的名称。 指定服务器上的默认数据库用于对查询进行分析。
...
4.  在中 `Name` ，用**SalesInformationByTerritory**替换默认名称。
...
3.  在中 `Name` ，用**SalesInformationByTerritory**替换默认名称。
