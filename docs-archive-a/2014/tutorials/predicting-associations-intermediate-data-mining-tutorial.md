...
4.  在网格中，单击 "**源**" 下的空单元格，然后选择 "**预测函数"。** 在 "**字段**" 下的单元中，选择 `PredictAssociation` 。  
  
     还可以使用**predict**函数预测关联。 如果执行此操作，请确保选择采用表列作为参数的**Predict**函数的版本。  
  
5.  在 "**挖掘模型**" 窗格中，选择嵌套表 `vAssocSeqLineItems` ，并将其拖到网格中的 "**条件/参数**" 框 `PredictAssociation` 。  
  
     通过拖放表和列名称可以生成没有语法错误的复杂语句。 但是，它将替换单元的当前内容，其中包括函数的其他可选参数 `PredictAssociation` 。 为了查看其他参数，可以将函数的另一个实例临时添加到网格供参考。  
...
5.  在 "**挖掘模型**" 窗格中，选择嵌套表 `vAssocSeqLineItems` ，并将其拖到网格中的 "**条件/参数**" 框 `PredictAssociation` 。 在 `,3` 嵌套表名称后键入，就像在前面的过程中一样。  
  
6.  在 "**单独查询输入**" 对话框中，单击 " **vAssoc Seq 项**" 旁边的 "**值**" 框，然后单击 " ** ( ... ) ** " 按钮。  
  
7.  在 "**嵌套表输入**" 对话框中，在 `Touring Tire` "**键列**" 窗格中选择，然后单击 "**添加**"。  
...
6.  在 "**嵌套表输入**" 对话框中，在 `Touring Tire Tube` "**键列**" 窗格中选择，然后单击 "**添加**"。  
  
7.  在网格的函数行中 `PredictAssociation` ，单击 "**条件/参数**" 框，然后更改参数以添加参数，INCLUDE_STATISTICS。  
...
11. 在网格中添加一个新行。 对于 "**源**"，选择 "**预测函数**"，为 "**字段**" 选择 `PredictAssociation` 。  
  
12. 将 vAssocSeqLineItems 拖到行的 "**条件/参数**" 框中 `PredictAssociation` 。 单击 "**条件/参数**" 框的末尾，然后键入以下文本：`INCLUDE_STATISTICS,3`  
