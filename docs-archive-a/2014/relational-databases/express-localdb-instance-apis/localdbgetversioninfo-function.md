...
## <a name="details"></a>详细信息  
 引入 `struct` 大小参数 (*lpVersionInfoSize*) 的基本原理是使 API 能够返回不同版本的**LocalDBVersionInfostruct**，从而有效地实现向前和向后兼容性。  
  
 如果 `struct` size 参数 (*LpVersionInfoSize*) 与**LocalDBVersionInfostruct**的已知版本大小匹配，则返回该版本的 `struct` 。 否则，返回 LOCALDB_ERROR_INVALID_PARAMETER。  
