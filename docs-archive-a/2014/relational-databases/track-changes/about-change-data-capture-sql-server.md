...
## <a name="understanding-change-data-capture-and-the-capture-instance"></a>了解变更数据捕获和捕获实例  
 ...
 与捕获实例关联的所有对象都是在启用变更数据捕获的数据库的变更数据捕获架构中创建的。 捕获实例名称的要求是：必须是有效的对象名，并且在数据库捕获实例中是唯一的。 默认情况下，该名称即为源表的 \<*schema name*_*table name*>。 它的关联更改表的命名方式为：在捕获实例名称后面追加 `_CT`。 用于查询所有更改的函数的命名方式为：在捕获实例名称后面追加 `fn_cdc_get_all_changes_`。 如果将捕获实例配置为支持 `net changes` ，则 `net_changes` 还会通过将**fn_cdc_get_net_changes \_ **追加到捕获实例名称，来创建和命名查询函数。  
