...
1.  在设计视图中，右键单击图例，然后打开 " _\<report item->_ **属性**" 页。  
  
2.  在 **“位置”** 中，通过单击指定图例要在视区中显示的相对位置。  
  
3.  若要在视区之外显示图例，请选择 "在** \<report item> 视区外显示**"。  
...
3.  选择 "**使用数据实现数据的可视化效果**" \<rule type\> 选项。 若要使用分布选项，必须根据与层关联的分析数据，在 **“分布”** 页上创建范围。  
...
3.  验证是否选择了 "**使用实现数据的可视化效果**" \<*rule type*> 。  
...
3.  选择要使用的颜色规则选项。 若要在地图图例或色阶中显示项，您必须使用选项选择一个**可视化数据** \<rule type> 。  
