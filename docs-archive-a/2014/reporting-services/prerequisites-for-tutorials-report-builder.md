...
-   对 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)] 报表生成器的访问权限。 可以使用报表生成器的独立版本或 ClickOnce 版本（可从报表管理器或 SharePoint 站点获取），来运行报表生成器。 ClickOnce 版本只有第一步（如何打开报表生成器）不同。  
     ...
     若要使用报表管理器，请打开报表管理器并单击 "**报表生成器**"。 默认情况下，报表管理器的 URL 是 http:// \<*servername*> /reports。  
...
-   对于包括地图的教程，报表服务器必须配置为支持将 Bing 地图作为背景。 有关详细信息，请[Plan for Map Report Support](plan-for-map-report-support.md)参阅 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] msdn.microsoft.com 上[联机丛书](https://go.microsoft.com/fwlink/?LinkId=154888)的文档中的规划地图报表支持。  
