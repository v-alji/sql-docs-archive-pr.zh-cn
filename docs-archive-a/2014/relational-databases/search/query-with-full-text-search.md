...
##  <a name="overview-of-the-full-text-predicates-contains-and-freetext"></a><a name="OV_ft_predicates"></a> (CONTAINS 和 FREETEXT 的全文谓词概述)   
...
-   使用 CONTAINS（或 CONTAINSTABLE）可搜索单个词和短语的精确或模糊（不太精确的）匹配项、在一定差别范围内的相近词或加权匹配项。 当使用 CONTAINS 时，必须指定至少一个搜索条件，该搜索条件须指定要搜索的文本以及确定匹配项的条件。  
     ...
     可以在搜索条件之间使用逻辑运算。 有关详细信息，请参阅本主题后面的[使用布尔运算符（AND、OR 和 NOT (IN CONTAINS AND CONTAINSTABLE) ](#Using_Boolean_Operators)。  
...
##  <a name="overview-of-the-full-text-functions-containstable-and-freetexttable"></a><a name="OV_ft_functions_CONTAINSTABLE_FREETEXTTABLE"></a>全文函数概述 (CONTAINSTABLE 和 FREETEXTTABLE)   
...
###  <a name="searching-for-specific-word-or-phrase-simple-term"></a><a name="Simple_Term"></a>搜索特定的词或短语 (简单术语)   
...
###  <a name="performing-prefix-searches-prefix-term"></a><a name="Prefix_Term"></a> (前缀术语) 执行前缀搜索  
...
###  <a name="searching-for-inflectional-forms-of-a-specific-word-generation-term"></a><a name="Inflectional_Generation_Term"></a>搜索特定词 (生成词) 的变形形式  
...
###  <a name="searching-for-words-or-phrases-using-weighted-values-weighted-term"></a><a name="Weighted_Term"></a>使用加权值搜索词或短语 (加权术语)   
