...
## <a name="example"></a>示例  
 若要运行以下代码，请添加服务器名称来代替每个 \<*servername*> 。 如果不是默认位置，请将路径更新为指向安装位置的路径。 下面的代码示例会遍历 [MSReportServer_ConfigurationSetting](msreportserver-configurationsetting-class.md) 类的每个属性，并向控制台输出每个属性的名称及其属性值。  
