...
13. 在 "**值**" 列表中，右键单击 `CountryRegionID` 并单击 " **Sum** " 选项。  
...
##  <a name="7-use-an-indicator-to-show-sales-comparison"></a><a name="Indicator"></a>7. 使用指示器显示销售额比较  
 添加一个新列，并使用指示器显示某个人的年初至今 (本年迄今) 购买高于还是低于平均 YTD 采购。 **Round** 函数从值中去掉小数部分。  
...
##  <a name="optional-format-date-column"></a><a name="DateFormat"></a> (可选) 格式日期列  
