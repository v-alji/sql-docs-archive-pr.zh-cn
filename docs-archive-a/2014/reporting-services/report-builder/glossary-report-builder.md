...
|aggregation|包含预先计算好的多维数据集数据的表或结构。|  
...
|复杂表达式 (complex expression)|不能表示为单个内置字段引用且在设计图面上显示为 <> 的表达式 \<Expr> 。|  
...
|Field — 字段|1.窗口或记录中存储单个数据值的区域。  2. 在报表生成器中，数据集字段表示来自数据连接的数值或非数值数据。 例如，销售额、总销售额、客户名称、数据库标识符、URL、图像和空间数据。 报表可具有三种类型的字段：数据集字段、数据集计算字段和内置字段。|  
...
|gauge|仪表是一种数据区域，该区域显示某个范围之内的数据，并且带有一个指向该范围内的某个特定值的指示器。|  
...
|参数|报表中的一种内置全局集合，使用户能够设置可根据报表数据、外观而变化并且可以连接相关报表的值。 报表参数是根据查询参数和数据集参数自动创建的，并且可由报表作者手动更改。|  
...
|精准率|可以存储的十进制数的最大总位数，包括小数点左边和右边的位数。|  
