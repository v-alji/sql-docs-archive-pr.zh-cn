...
# <a name="create-modify-and-drop-spatial-indexes"></a>创建、修改和删除空间索引
  空间索引可以更高效地对或数据类型的列执行某些操作， `geometry` `geography` () *空间列*。 可对空间数据列指定多个空间索引。 这非常有用，例如，对单一列中的不同分割参数建立索引时，就是如此。  
...
11. 在 **“空间”** 页上，指定要用于索引的空间属性的值。  
     ...
     对类型列创建索引时 `geometry` ，必须指定** (*`X-min`* 、 *`Y-min`*) **和** (*`X-max`* ， *`Y-max`*) **边界框的坐标。 对于类型列上的索引 `geography` ，指定 "**地理网格**" 分割方案后，边界框字段变为只读，因为地理网格分割不使用边界框。  
