...
|`WSID`|SSPROP_INIT_WSID|工作站标识符。|  
 ...
 属性使用必须符合其作用域中允许的语法。  例如， `WSID` 使用大括号 (`{}`) 的引号字符，并 `Application Name` 使用单个 (`'`) 或 double (`"`) 引号字符。 只有字符串属性可以加引号。 尝试将整数或枚举属性用引号引起来将导致“连接字符串不符合 OLE DB 规范”错误。  
