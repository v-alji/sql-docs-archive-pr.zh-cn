...
# <a name="the-oracle-cdc-service"></a>Oracle CDC 服务
 ...
 Oracle CDC 服务是使用 Oracle CDC 服务配置控制台创建的，或者是通过内置于 xdbcdcsvc.exe 程序中的命令行接口定义的。 在这两种情况下，创建的每个 Oracle CDC 服务都与单个实例相关联 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] (可以使用**AlwaysOn**安装) 程序对该实例进行群集或镜像， (连接字符串和访问凭据) 是服务配置的一部分。  
