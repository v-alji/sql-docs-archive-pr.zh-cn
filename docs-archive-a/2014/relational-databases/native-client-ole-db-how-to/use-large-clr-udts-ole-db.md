...
ms.locfileid: "87687840"
---
# <a name="use-large-clr-udts-ole-db"></a><span data-ttu-id="e4f55-102">使用大型 CLR UDT (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="e4f55-102">Use Large CLR UDTs (OLE DB)</span></span>
  <span data-ttu-id="e4f55-103">此示例演示如何从结果集中提取具有大型用户定义类型的行。</span><span class="sxs-lookup"><span data-stu-id="e4f55-103">This sample shows how to fetch rows with large user-defined types from a result set.</span></span> <span data-ttu-id="e4f55-104">有关详细信息，请参阅[大型 CLR 用户定义类型 &#40;OLE DB&#41;](../native-client/ole-db/large-clr-user-defined-types-ole-db.md)。</span><span class="sxs-lookup"><span data-stu-id="e4f55-104">For more information, see [Large CLR User-Defined Types &#40;OLE DB&#41;](../native-client/ole-db/large-clr-user-defined-types-ole-db.md).</span></span> <span data-ttu-id="e4f55-105">此示例适用于 [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 或更高版本。</span><span class="sxs-lookup"><span data-stu-id="e4f55-105">This sample works with [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] or later.</span></span>  
  
## <a name="example"></a><span data-ttu-id="e4f55-106">示例</span><span class="sxs-lookup"><span data-stu-id="e4f55-106">Example</span></span>  
 <span data-ttu-id="e4f55-107">此示例包含两个项目。</span><span class="sxs-lookup"><span data-stu-id="e4f55-107">This sample contains two projects.</span></span> <span data-ttu-id="e4f55-108">其中一个项目基于 C# 源代码创建一个程序集 (DLL)。</span><span class="sxs-lookup"><span data-stu-id="e4f55-108">One project creates an assembly (DLL) from C# source code.</span></span> <span data-ttu-id="e4f55-109">该程序集包含 CLR 类型。</span><span class="sxs-lookup"><span data-stu-id="e4f55-109">This assembly contains the CLR type.</span></span> <span data-ttu-id="e4f55-110">数据库中将添加一个表。</span><span class="sxs-lookup"><span data-stu-id="e4f55-110">A table will be added to the database.</span></span> <span data-ttu-id="e4f55-111">该表中的一个列将属于该程序集中定义的一个类型。默认情况下，此示例将使用 master 数据库。</span><span class="sxs-lookup"><span data-stu-id="e4f55-111">A column in the table will be of a type defined in the assembly, By default, this sample will use the master database.</span></span> <span data-ttu-id="e4f55-112">第二个项目是一个本机 C 应用程序，用来读取表中的数据。</span><span class="sxs-lookup"><span data-stu-id="e4f55-112">The second project is a native C application that reads data from the table.</span></span>  
  
 <span data-ttu-id="e4f55-113">将第一个 （C#） 代码列表编译为 DLL。</span><span class="sxs-lookup"><span data-stu-id="e4f55-113">Compile the first (C#) code listing to a DLL.</span></span>  <span data-ttu-id="e4f55-114">然后，将该 DLL 复制到 C 驱动器的根目录。</span><span class="sxs-lookup"><span data-stu-id="e4f55-114">Then, copy the DLL to the root directory of the C drive.</span></span>  
  
 <span data-ttu-id="e4f55-115">执行第二个 ([!INCLUDE[tsql](../../includes/tsql-md.md)]) 代码列表，将该程序集添加到 master 数据库。</span><span class="sxs-lookup"><span data-stu-id="e4f55-115">Execute the second ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing to add the assembly to the master database.</span></span>  
  
 <span data-ttu-id="e4f55-116">使用 ole32.lib 和 oleaut32.lib 编译并执行第三个 (C++) 代码列表。</span><span class="sxs-lookup"><span data-stu-id="e4f55-116">Compile with ole32.lib oleaut32.lib and execute the third (C++) code listing.</span></span> <span data-ttu-id="e4f55-117">此应用程序连接到您的计算机上默认的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例。</span><span class="sxs-lookup"><span data-stu-id="e4f55-117">This application connects to your computer's default [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="e4f55-118">在某些 Windows 操作系统上，您需要将 (localhost) 或 (local) 更改为您的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例的名称。</span><span class="sxs-lookup"><span data-stu-id="e4f55-118">On some Windows operating systems, you will need to change (localhost) or (local) to the name of your [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="e4f55-119">若要连接到命名实例，请将连接字符串从 L"(local)" 更改为 L"(local)\\\name"，其中 name 是命名实例。</span><span class="sxs-lookup"><span data-stu-id="e4f55-119">To connect to a named instance, change the connection string from L"(local)" to L"(local)\\\name" , where name is the named instance.</span></span> <span data-ttu-id="e4f55-120">默认情况下，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express 安装在命名实例中。</span><span class="sxs-lookup"><span data-stu-id="e4f55-120">By default, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express installs to a named instance.</span></span> <span data-ttu-id="e4f55-121">请确保 INCLUDE 环境变量包含包含 sqlncli.msi 的目录。</span><span class="sxs-lookup"><span data-stu-id="e4f55-121">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="e4f55-122">执行第四个 ([!INCLUDE[tsql](../../includes/tsql-md.md)]) 代码列表，从 master 数据库删除该程序集。</span><span class="sxs-lookup"><span data-stu-id="e4f55-122">Execute the fourth ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing to delete the assembly to the master database.</span></span>  
