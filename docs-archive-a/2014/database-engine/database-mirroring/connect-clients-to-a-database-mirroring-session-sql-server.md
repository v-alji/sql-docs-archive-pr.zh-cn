...
> [!IMPORTANT]  
>  将协议前缀与 `Server` () 的属性绑定 `Server=tcp:` *\<servername>* 不兼容**网络**属性，同时在这两个位置指定协议可能会导致错误。 因此，建议连接字符串使用**Network**属性指定协议，并且仅在属性中指定服务器名称 `Server` (`"Network=dbmssocn; Server=` *\<servername>* `"`) 。  
