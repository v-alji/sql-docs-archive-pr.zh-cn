...
##  <a name="use-a-fixed-port-for-a-default-or-named-instance-of-analysis-services"></a><a name="bkmk_fixed"></a>为 Analysis Services 的默认实例或命名实例使用固定端口  
 ...
 如果你使用的是命名实例，则可以分配一个固定端口，但不更改指定服务器名称 (具体而言，你可以使用 \<servername\instancename> 连接到侦听固定端口) 上的命名实例。 这仅适用于 SQL Server Browser 服务正在运行并且您已取消阻止了该服务正在侦听的端口的情况。 SQL Server Browser 服务将提供基于的固定端口的重定向 \<servername\instancename> 。 只要您为 SQL Server Browser 服务以及侦听固定端口的 Analysis Services 的命名实例开放端口，SQL Server Browser 服务就会解析与命名实例的连接。  
