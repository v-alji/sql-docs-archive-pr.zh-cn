...
## <a name="return"></a>返回  
 `SqlGeometry`基于表达式类型返回或的空间对象 `SqlGeography` 。 有关 `SqlGeometry` 和 `SqlGeography` 空间数据类型的详细信息，请参阅[空间数据类型概述](../../relational-databases/spatial/spatial-data-types-overview.md)。  
