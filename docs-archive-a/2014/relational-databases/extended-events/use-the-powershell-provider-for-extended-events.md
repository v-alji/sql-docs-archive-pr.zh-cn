...
-   在命令提示符处，键入 `sqlps`，然后按 Enter。 键入 `cd xevent`，然后按 Enter。 在这里，你可以使用**cd**和 `dir` 命令 (或**设置位置**和**get-childitem** cmdlet) 导航到服务器名称和实例名称。  
...
-   脚本必须从 PS SQLSERVER： \\> 提示符下运行 (可通过 `sqlps` 在命令提示符处键入) 来运行。  
