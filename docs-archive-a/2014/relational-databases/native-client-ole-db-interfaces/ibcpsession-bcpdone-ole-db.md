...
ms.locfileid: "87590201"
---
# <a name="ibcpsessionbcpdone-ole-db"></a><span data-ttu-id="d39cf-102">IBCPSession::BCPDone (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="d39cf-102">IBCPSession::BCPDone (OLE DB)</span></span>
  <span data-ttu-id="d39cf-103">提交要发送到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的剩余行。</span><span class="sxs-lookup"><span data-stu-id="d39cf-103">Commits the remaining rows to be sent to [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].</span></span>  
  
## <a name="syntax"></a><span data-ttu-id="d39cf-104">语法</span><span class="sxs-lookup"><span data-stu-id="d39cf-104">Syntax</span></span>  
...
## <a name="remarks"></a><span data-ttu-id="d39cf-105">备注</span><span class="sxs-lookup"><span data-stu-id="d39cf-105">Remarks</span></span>  
 <span data-ttu-id="d39cf-106">在调用 BCPDone 方法之后，不能对 [IBCPSession](ibcpsession-ole-db.md) 接口调用其他操作\*\*\*\*。</span><span class="sxs-lookup"><span data-stu-id="d39cf-106">No other operation can be called on the [IBCPSession](ibcpsession-ole-db.md) interface after calling the **BCPDone** method.</span></span> <span data-ttu-id="d39cf-107">唯一的可能性是调用 [IBCPSession::BCPInit](ibcpsession-bcpinit-ole-db.md) 方法以启动新的大容量复制操作。</span><span class="sxs-lookup"><span data-stu-id="d39cf-107">The only possibility is to call the [IBCPSession::BCPInit](ibcpsession-bcpinit-ole-db.md) method to initiate a new bulk copy operation.</span></span> <span data-ttu-id="d39cf-108">这类似于调用 [IRowsetFastLoad::Commit](irowsetfastload-commit-ole-db.md) 方法。</span><span class="sxs-lookup"><span data-stu-id="d39cf-108">This is similar to calling the [IRowsetFastLoad::Commit](irowsetfastload-commit-ole-db.md) method.</span></span>  
  
## <a name="return-code-values"></a><span data-ttu-id="d39cf-109">返回代码值</span><span class="sxs-lookup"><span data-stu-id="d39cf-109">Return Code Values</span></span>  
 <span data-ttu-id="d39cf-110">S_OK</span><span class="sxs-lookup"><span data-stu-id="d39cf-110">S_OK</span></span>  
 <span data-ttu-id="d39cf-111">方法成功。</span><span class="sxs-lookup"><span data-stu-id="d39cf-111">The method succeeded.</span></span>  
  
 <span data-ttu-id="d39cf-112">E_UNEXPECTED</span><span class="sxs-lookup"><span data-stu-id="d39cf-112">E_UNEXPECTED</span></span>  
 <span data-ttu-id="d39cf-113">意外调用了该方法。</span><span class="sxs-lookup"><span data-stu-id="d39cf-113">The call to the method was unexpected.</span></span> <span data-ttu-id="d39cf-114">例如，在调用该方法之前，未调用 BCPInit 方法\*\*\*\*。</span><span class="sxs-lookup"><span data-stu-id="d39cf-114">For example, the **BCPInit** method was not called before calling this method.</span></span>  
  
## <a name="example"></a><span data-ttu-id="d39cf-115">示例</span><span class="sxs-lookup"><span data-stu-id="d39cf-115">Example</span></span>  
 <span data-ttu-id="d39cf-116">该示例演示如何使用 IBCPSession 接口\*\*\*\*。</span><span class="sxs-lookup"><span data-stu-id="d39cf-116">This sample shows how to use the **IBCPSession** interface.</span></span>  
  
 <span data-ttu-id="d39cf-117">在运行此示例之前，必须执行以下 [!INCLUDE[tsql](../../includes/tsql-md.md)]：</span><span class="sxs-lookup"><span data-stu-id="d39cf-117">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] must be executed before running this sample:</span></span>  
...
```  
 ...
 <span data-ttu-id="d39cf-118">此示例连接到 master 数据库。</span><span class="sxs-lookup"><span data-stu-id="d39cf-118">The sample connects to the master database.</span></span>  
  
 <span data-ttu-id="d39cf-119">当示例运行时，它将在项目目录中创建 outfile.dat。</span><span class="sxs-lookup"><span data-stu-id="d39cf-119">When the sample runs, it creates outfile.dat in the project directory.</span></span> <span data-ttu-id="d39cf-120">outfile.dat 包含在本机（二进制）模式下从表中复制的数据。</span><span class="sxs-lookup"><span data-stu-id="d39cf-120">The outfile.dat contains the data copied from the table in native (binary) mode.</span></span>  
  
 <span data-ttu-id="d39cf-121">可以通过以下命令，使用 BCP 将此数据添加回表中：</span><span class="sxs-lookup"><span data-stu-id="d39cf-121">You could use BCP to add this data back into the table with the following command:</span></span>  
  
 <span data-ttu-id="d39cf-122">bcp master..fltest in outfile.dat -n -T -S\*\* server\*\* \*\*</span><span class="sxs-lookup"><span data-stu-id="d39cf-122">**bcp master..fltest in outfile.dat -n -T -S** *server*</span></span>  
  
 <span data-ttu-id="d39cf-123">当编译此示例时，您需要指定 sqlncli11.lib。</span><span class="sxs-lookup"><span data-stu-id="d39cf-123">You will need to specify sqlncli11.lib when compiling this sample.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="d39cf-124">另请参阅</span><span class="sxs-lookup"><span data-stu-id="d39cf-124">See Also</span></span>  
 <span data-ttu-id="d39cf-125">[IBCPSession &#40;OLE DB&#41;](ibcpsession-ole-db.md) </span><span class="sxs-lookup"><span data-stu-id="d39cf-125">[IBCPSession &#40;OLE DB&#41;](ibcpsession-ole-db.md) </span></span>  
 [<span data-ttu-id="d39cf-126">执行大容量复制操作</span><span class="sxs-lookup"><span data-stu-id="d39cf-126">Performing Bulk Copy Operations</span></span>](../native-client/features/performing-bulk-copy-operations.md)  
