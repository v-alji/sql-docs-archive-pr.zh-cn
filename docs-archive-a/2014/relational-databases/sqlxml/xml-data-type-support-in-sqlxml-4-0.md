...
-   SQLXML 4.0 不保留 XML 声明处理指令 (例如，基于的行为 ) [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 。 而是将 XML 声明视为针对 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] XML 分析器的指令，并且，在将数据转换到 `xml` 数据类型之后，此声明的属性（version、encoding 和 standalone）将丢失。 XML 数据在内部存储为 UCS-2。 将保留 XML 实例中的所有其他处理指令；在 `xml` 列中允许使用这些指令，并且 SQLXML 支持它们。  
