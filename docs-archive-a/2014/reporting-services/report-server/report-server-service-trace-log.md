...
> [!NOTE]
>  在早期版本中，每个应用程序都具有一个跟踪日志文件，因此具有多个跟踪日志文件。 以下文件已过时，不再在 [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 和更高版本中创建： ReportServerWebApp_ *\<timestamp>* 日志、ReportServer_ *\<timestamp>* 和 ReportServerService_main_ *\<timestamp>* 。
