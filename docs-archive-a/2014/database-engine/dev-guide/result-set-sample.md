...
ms.locfileid: "87591567"
---
# <a name="result-set-sample"></a><span data-ttu-id="f2f2f-102">结果集示例</span><span class="sxs-lookup"><span data-stu-id="f2f2f-102">Result Set Sample</span></span>
  <span data-ttu-id="f2f2f-103">有时，在通读查询结果时能够执行命令（而不需要打开新的连接并将所有结果读入内存）很有用。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-103">Sometimes it is useful to be able to execute commands while reading through results of a query without opening a new connection and without reading all the results into memory.</span></span> <span data-ttu-id="f2f2f-104">ADO.NET 2.0 中的多个活动的结果集 (MARS) 功能就是一种能够帮助您实现以上操作的技术。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-104">The Multiple Active Result Set (MARS) feature in ADO.NET 2.0 is a technology that can help you achieve that.</span></span> <span data-ttu-id="f2f2f-105">目前，用于服务器端编程的进程内的提供程序不能实现 MARS。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-105">Currently, MARS is not implemented for the in-process provider used for server-side programming.</span></span> <span data-ttu-id="f2f2f-106">若要消除此限制，可以使用服务器端游标。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-106">To work around this limitation, you can use server-side cursors.</span></span> <span data-ttu-id="f2f2f-107">此示例说明如何使用服务器端游标解决对服务器端编程缺少 MARS 支持的问题。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-107">This sample demonstrates how to use server-side cursors to work around the lack of MARS support for server-side programming.</span></span>  
  
> [!CAUTION]  
>  <span data-ttu-id="f2f2f-108">使用服务器端游标要消耗大量的服务器资源，并且有时会阻碍 Microsoft SQL Server 中的查询优化器增强查询的性能。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-108">Using server-side cursors is expensive in terms of server resources and can sometimes prevent the query optimizer in Microsoft SQL Server from enhancing the performance of your queries.</span></span> <span data-ttu-id="f2f2f-109">因此，可以考虑重新编写代码来尽可能使用 `JOINs`。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-109">Therefore, you might consider rewriting your code to use `JOINs` whenever possible.</span></span>  
  
 <span data-ttu-id="f2f2f-110">除了可以在结果集中前后移动以外，此类的 API 与数据读取器相似；并且即使结果集处于打开状态，也可以在连接时发出其他命令。此实现过程经过较大程度的简化，可令该示例易于理解。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-110">The API for this class is similar to a data reader except that you can move forward or backward throughout the result set, and other commands can be issued on the connection while the result set is open.This implementation is highly simplified to make the sample easy to understand.</span></span> <span data-ttu-id="f2f2f-111">更有效的实现将提取多行来避免提取的每行的数据库周转。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-111">A more efficient implementation would fetch multiple rows to avoid a database turnaround per row fetched.</span></span> <span data-ttu-id="f2f2f-112">与用查询的所有结果填充数据集比较，使用此类可以大大减少内存需求量，这对于服务器端编程是非常重要的。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-112">Using this class can have a significantly smaller memory footprint than filling a dataset with all the results of a query, which is very important for server-side programming.</span></span> <span data-ttu-id="f2f2f-113">此示例还说明如何使用“允许部分受信任的调用方”属性来指示结果集程序集是可以安全地从其他程序集进行调用的库。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-113">This sample also demonstrates using the "Allow partially trusted callers" attribute to indicate that the Result Set assembly is a library which may be safely called from other assemblies.</span></span> <span data-ttu-id="f2f2f-114">与使用不安全的权限注册调用程序集相比，此方法稍微复杂一些，但要安全许多。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-114">This approach is a little more complex but much more secure than registering the calling assembly using the unsafe permission.</span></span> <span data-ttu-id="f2f2f-115">该方法更安全的原因是：它将调用程序集注册为安全的程序集，调用程序集将限制相关资源脱离服务器并防止对服务器的完整性造成损害。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-115">It is more secure because registering the calling assembly as safe the calling assembly limits affecting resources off the server and prevents damage to the integrity of the server.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="f2f2f-116">先决条件</span><span class="sxs-lookup"><span data-stu-id="f2f2f-116">Prerequisites</span></span>  
 <span data-ttu-id="f2f2f-117">若要创建和运行此项目，必须安装下列软件：</span><span class="sxs-lookup"><span data-stu-id="f2f2f-117">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="f2f2f-118">或 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-118">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="f2f2f-119">可以从 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express 文档和示例[网站](https://www.microsoft.com/sql-server/sql-server-editions-express)免费获取 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express</span><span class="sxs-lookup"><span data-stu-id="f2f2f-119">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="f2f2f-120">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]开发人员[网站](https://go.microsoft.com/fwlink/?linkid=62796)提供的 AdventureWorks 数据库</span><span class="sxs-lookup"><span data-stu-id="f2f2f-120">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="f2f2f-121">.NET Framework SDK 2.0 或更高版本，或 Microsoft Visual Studio 2005 或更高版本。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-121">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="f2f2f-122">您可以免费获取 .NET Framework SDK。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-122">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="f2f2f-123">此外，还必须满足以下条件：</span><span class="sxs-lookup"><span data-stu-id="f2f2f-123">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="f2f2f-124">使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例必须已启用 CLR 集成。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-124">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="f2f2f-125">若要启用 CLR 集成，请执行以下步骤：</span><span class="sxs-lookup"><span data-stu-id="f2f2f-125">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="f2f2f-126">启用 CLR 集成</span><span class="sxs-lookup"><span data-stu-id="f2f2f-126">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="f2f2f-127">执行以下 [!INCLUDE[tsql](../../includes/tsql-md.md)] 命令：</span><span class="sxs-lookup"><span data-stu-id="f2f2f-127">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="f2f2f-128">若要启用 CLR，您必须具有 `ALTER SETTINGS` 服务器级别权限，`sysadmin` 和 `serveradmin` 固定服务器角色的成员隐式拥有该权限。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-128">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="f2f2f-129">必须在您使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例上安装 AdventureWorks 数据库。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-129">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="f2f2f-130">如果你不是要使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例的管理员，必须请管理员授予你 **CreateAssembly** 权限，才能完成安装。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-130">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="f2f2f-131">生成示例</span><span class="sxs-lookup"><span data-stu-id="f2f2f-131">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="f2f2f-132">按照以下说明创建和运行该示例：</span><span class="sxs-lookup"><span data-stu-id="f2f2f-132">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="f2f2f-133">打开 Visual Studio 或 .NET Framework 命令提示符。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-133">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="f2f2f-134">如有必要，为您的示例创建目录。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-134">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="f2f2f-135">对于此示例，我们将使用 C:\MySample。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-135">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="f2f2f-136">在命令提示符处执行以下命令，创建强名称密钥文件。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-136">Create a strong  name key file by executing the following at the command prompt.</span></span>  
  
## <a name="sample-code"></a><span data-ttu-id="f2f2f-137">代码示例</span><span class="sxs-lookup"><span data-stu-id="f2f2f-137">Sample Code</span></span>  
 <span data-ttu-id="f2f2f-138">下面是此示例的代码列表。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-138">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="f2f2f-139">此代码用于库 `ResultSet.`</span><span class="sxs-lookup"><span data-stu-id="f2f2f-139">This is the code for the Library `ResultSet.`</span></span>  
  
 <span data-ttu-id="f2f2f-140">C#</span><span class="sxs-lookup"><span data-stu-id="f2f2f-140">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="f2f2f-141">此代码用于测试库 `ResultSet.`</span><span class="sxs-lookup"><span data-stu-id="f2f2f-141">This is the code to test the library `ResultSet.`</span></span>  
  
 <span data-ttu-id="f2f2f-142">C#</span><span class="sxs-lookup"><span data-stu-id="f2f2f-142">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="f2f2f-143">这是 [!INCLUDE[tsql](../../includes/tsql-md.md)] 安装脚本 (`Install.sql`)，该脚本部署程序集并创建存储过程。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-143">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assemblies and creates the stored procedures.</span></span>  
...
```  
 ...
 <span data-ttu-id="f2f2f-144">这是用于测试该示例的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 测试脚本 (`test.sql`)。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-144">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] test script (`test.sql`), which tests the sample.</span></span>  
...
```  
 ...
 <span data-ttu-id="f2f2f-145">下面的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 从数据库中删除程序集和存储过程。</span><span class="sxs-lookup"><span data-stu-id="f2f2f-145">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assemblies and stored procedures from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="f2f2f-146">另请参阅</span><span class="sxs-lookup"><span data-stu-id="f2f2f-146">See Also</span></span>  
 [<span data-ttu-id="f2f2f-147">公共语言运行时 (CLR) 集成的使用方案和示例</span><span class="sxs-lookup"><span data-stu-id="f2f2f-147">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
