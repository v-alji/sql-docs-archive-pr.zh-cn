...
-   `/Book/*/Title` 其中，通配符 (`*`) 是指定在表达式中间的。  
 ...
 例如，以下查询将返回表的 "类型" 列中存储的摘要信息 `CatalogDescription``xml` `ProductModel` 。 只有当产品型号的目录说明中还存储 <`Features`> 说明时，该查询才会返回 <`Summary`> 信息。  
...
### <a name="property-secondary-index"></a>PROPERTY 辅助索引  
 从单个 XML 实例检索一个或多个值的查询适用 PROPERTY 索引。 如果使用类型** ( # B1**方法的值检索对象属性 `xml` ，并且对象的主键值已知，则会发生这种情况。  
