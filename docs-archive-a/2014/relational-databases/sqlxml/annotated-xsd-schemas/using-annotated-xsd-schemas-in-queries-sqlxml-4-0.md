...
```  
 ...
 除了在 `mapping-schema` 模板中的元素上指定属性外 **\<sql:xpath-query>** (当存在 XPath 查询) 或 **\<updg:sync>** updategram 中的元素时，你可以执行以下操作：  
  
-   在 `mapping-schema` **\<ROOT>** 模板 (全局声明) 的元素上指定属性。 这样，该映射架构就成为默认架构，并将由无显式 `mapping-schema` 批注的所有 XPath 和 updategram 节点使用。  
