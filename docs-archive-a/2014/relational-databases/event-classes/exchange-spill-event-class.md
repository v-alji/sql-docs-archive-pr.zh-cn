...
# <a name="exchange-spill-event-class"></a>Exchange Spill 事件类
 ...
 一般情况下，生成此类范围扫描的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 查询包含许多 BETWEEN 运算符，其中每个运算符会从表或索引中选择某一范围的行。 或者，可以使用表达式（如 (T）获取多个范围。 a > 10 和 T. \< 20) OR (T.a > 100 和 \< 120) 。 此外，查询计划必须要求按顺序扫描这些范围，原因是对 T.a 应用了 ORDER BY 子句，或者计划中的迭代器要求它按排序顺序处理元组。  
