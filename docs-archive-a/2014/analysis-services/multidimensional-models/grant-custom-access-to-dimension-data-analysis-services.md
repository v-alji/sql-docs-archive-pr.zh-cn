...
> [!NOTE]  
>  如果您定义了一个空集 ({} 属性成员) ，则该属性的成员将对数据库角色可见。 缺少允许集不会被解释为空集。  
 ...
 **启用直观合计**  
 ...
 默认情况下，VisualTotals 属性被禁用 (设置为 `False`) 。 此默认设置可以最大程度地利用性能，因为 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 可以快速计算所有单元值的合计值，而不必花时间选择要计算的单元值。  
