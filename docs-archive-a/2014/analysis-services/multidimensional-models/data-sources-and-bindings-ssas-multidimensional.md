...
|Integer|32 位带符号整数。 此数据类型映射到 [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 中的 Int32 数据类型和 OLE DB 中的 DBTYPE_I4 数据类型。|  
...
|SmallInt|16 位带符号整数。 此数据类型映射到 [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 中的 Int16 数据类型和 OLE DB 中的 DBTYPE_I2 数据类型。|  
|TinyInt|8 位有符号整数。 此数据类型映射到 [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 中的 SByte 数据类型和 OLE DB 中 DBTYPE_I1 数据类型。<br /><br /> 注意：如果数据源包含的字段属于 tinyint 数据类型，并且 AutoIncrement 属性设置为 True，则它们会在数据源视图中转换成整数。|  
|UnsignedBigInt|64 位无符号整数。 此数据类型映射到 [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 中的 UInt64 数据类型和 OLE DB 中的 DBTYPE_UI8 数据类型。|  
...
### <a name="definition-of-the-out-of-line-binding-type"></a>外部绑定类型的定义  
 ...
 为其指定绑定的每个对象由 ID 为的 XML 元素标识 \<*object*> (例如， `DimensionID`) 。 使用窗体 ID 明确地标识对象后 \<*object*> ，可以标识为其指定绑定的元素，该元素通常为 `Source` 。 有一个常见情况需要注意，即 `Source` 为 `DataItem` 的属性，这属于属性中的列绑定。 在这种情况下，您不需要指定 `DataItem` 标记，而只需要简单地指定 `Source` 属性，就如同该属性直接位于要绑定的列上一样。  
