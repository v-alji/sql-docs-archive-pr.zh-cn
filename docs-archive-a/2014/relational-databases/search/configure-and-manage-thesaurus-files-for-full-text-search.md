...
-   标注字符设置  
     ...
     对于给定的同义词库，所有搜索模式都是敏感的或不区分变体标记，如波形符 (**~**) 、锐音符号 (**？？？**) ，或元音**格式设置**为区分*大小*写或不*区分*重音 () 。 例如，假设指定了 "caf？" 模式。 替换为全文查询中的其他模式。 如果同义词库不区分重音，则全文搜索将替换模式 "caf？"。 和“cafe”。 如果同义词库区分重音，则全文搜索仅替换模式 "caf？"。 默认情况下，同义词库不区分重音。  
